...
namespace ControlPanel.WeiXin
{
	...
	{
		public static Dictionary<string, string> BindAdminOpenId;

		static WeiXinHelper()
		{
			WeiXinHelper.BindAdminOpenId = new Dictionary<string, string>();
		}
		...
		{
			bool qRCodeScanInfo = (new SendAllDao()).GetQRCodeScanInfo(AppID, IsClearAfterRead, out SCannerUserOpenID, out SCannerUserNickName, out UserHead);
		...
		{
			bool flag = (new SendAllDao()).UpdateMsgId(id, msgid, sendstate, sendcount, totalcount, returnjsondata);
			return flag;
		}
