        ...
        public HttpWebRequest GetWebRequest(string url, string method, string cerPassword = null, string cerPath = null)
        {
            ...
            if (url.Contains("https"))
            {
                ...
                try
                {
                    ...
                    {
                        byte[] array;
                        using (FileStream fileStream = new FileStream(cerPath, FileMode.Open, FileAccess.Read))
                        {
                            array = new byte[fileStream.Length];
                            fileStream.Read(array, 0, array.Length);
                            fileStream.Close();
                        }
                        X509Certificate2 value = new X509Certificate2(array, cerPassword, X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.PersistKeySet);
