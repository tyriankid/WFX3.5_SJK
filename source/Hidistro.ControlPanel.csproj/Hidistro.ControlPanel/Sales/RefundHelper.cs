	...
	public static class RefundHelper
	{
		public static bool CloseTransaction(OrderInfo order)
		{
			...
			if (flag)
			{
				...
				object[] orderId = new object[] { order.OrderId };
				EventLogs.WriteOperationLog(Privilege.EditOrders, string.Format(invariantCulture, "关闭了订单“{0}”", orderId));
			}
			return flag;
		}
		...
		public static bool DelRefundApply(string[] ReturnsIds, out int count)
		{
			...
			bool flag = true;
			count = 0;
			string[] returnsIds = ReturnsIds;
			for (int i = 0; i < (int)returnsIds.Length; i++)
			{
				string str = returnsIds[i];
