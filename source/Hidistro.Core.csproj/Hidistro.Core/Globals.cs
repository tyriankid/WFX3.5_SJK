...
namespace Hidistro.Core
{
	public static class Globals
	{
        ...
        private static string provCardNum = "";
		...
		public static string GetCurrentWXOpenId
		{
			get
			{
				...
				string currentWXOpenIdCookieName = Globals.GetCurrentWXOpenIdCookieName();
				HttpCookie httpCookie = HttpContext.Current.Request.Cookies.Get(currentWXOpenIdCookieName);
				...
				{
					text = HttpContext.Current.Session[currentWXOpenIdCookieName].ToString();
				}
				else if (httpCookie != null)
				{
					text = httpCookie.Value;
					HttpContext.Current.Session[currentWXOpenIdCookieName] = text;
				}
				return text;
			}
			set
			{
				...
				{
					HttpContext.Current.Session[currentWXOpenIdCookieName] = value;
					HttpCookie httpCookie = new HttpCookie(currentWXOpenIdCookieName);
		...
		public static string GetErpLink()
		{
			...
			string text = HiCryptographer.SignTopRequest(new SortedDictionary<string, string>
			{
				{
					"app_key",
					appKey
			...
			{
				"http://hierp.kuaidiantong.cn/ExpressBill/Allot?app_key=",
				appKey,
				"&timestamp=",
		...
		public static string GetStoreNameByUserIDAndName(int userid, string username, string openid, string storeName)
		{
			...
			if (userid > 0)
			{
				text = username + "的小店_" + userid.ToString();
			}
			else
			{
				text = username + "的小店";
		...
		public static void DelImgByFilePath(string savePath)
		{
			try
			{
				if (!savePath.ToLower().Contains("headlogo.jpg") && !savePath.ToLower().Contains("/distributorlogopic/") && !savePath.ToLower().Contains("/storage/master/logo/") && File.Exists(savePath))
				{
					string a = Path.GetExtension(savePath).ToLower();
					if (a == ".gif" || a == ".jpg" || a == ".png" || a == ".jpeg")
					{
			...
			{
				Globals.Debuglog(ex.ToString(), "_DebuglogDelFile.txt");
			}
		}
		...
		public static string GetReplaceStr(string objStr, string fromStr, string toStr)
		{
			...
			{
				result = objStr.Replace(fromStr, toStr);
			}
			return result;
		}
		...
		public static string FormatWXReplyContent(string str)
		{
			...
			else
			{
				str = str.Replace("<p>", "");
				str = str.Replace("</p>", "\n");
				result = HttpContext.Current.Server.HtmlDecode(str);
			}
			return result;
		}
		...
		public static int GetPoint(decimal money)
		{
			...
			else
			{
				...
				else if (!masterSettings.shopping_score_Enable)
				{
					result = 0;
				}
				else
				{
					decimal num2 = Math.Floor(money / masterSettings.PointsRate) * masterSettings.ShoppingScoreUnit;
					if (num2 > num)
					{
						result = num;
					}
					else
					{
						...
						{
							num3 += masterSettings.shopping_reward_Score;
							if (num3 > num)
							{
								num3 = num;
							}
		...
		public static void Debuglog(string log, string logname = "_Debuglog.txt")
		{
			lock (Globals.LockLog)
			{
				...
				catch (ThreadAbortException var_3_88)
				{
				}
