...
namespace Hidistro.Entities.Orders
{
	public class OrderInfo
	{
		...
		public LogisticsTools logisticsTools
		{
			get;
			set;
		}
        ...
        public decimal GetBalancePayMoneyTotal()
		{
			...
			foreach (LineItemInfo current in this.LineItems.Values)
			{
				if (current.Type == 0)
				{
					num += current.BalancePayMoney;
				}
			}
			num += this.BalancePayFreightMoneyTotal;
			num = decimal.Round(num, 2);
			return num;
		}
		...
		public decimal GetReturnAmount()
		{
			...
			foreach (LineItemInfo current in this.LineItems.Values)
			{
				if (current.Type == 0 && (current.OrderItemsStatus == OrderStatus.Returned || current.OrderItemsStatus == OrderStatus.Refunded || current.OrderItemsStatus == OrderStatus.Closed))
				{
