	...
	public static class DataMapper
	{
		...
		public static OrderInfo PopulateOrder(IDataRecord reader)
		{
			...
			else
			{
				...
				orderInfo.UpdateDate = (System.DateTime)reader["UpdateDate"];
                ...
                if (System.DBNull.Value != reader["SiteId"])
                {
				...
				orderInfo.CouponFreightMoneyTotal = (decimal)reader["CouponFreightMoneyTotal"];
				orderInfo.logisticsTools = (LogisticsTools)reader["LogisticsTools"];
				result = orderInfo;
			}
			return result;
		}
		...
		public static LineItemInfo PopulateLineItem(IDataRecord reader)
		{
			...
			else
			{
				...
				lineItemInfo.ItemsCommissionScale = (decimal)reader["ItemsCommissionScale"];
				lineItemInfo.SecondItemsCommissionScale = (decimal)reader["SecondItemsCommissionScale"];
				lineItemInfo.ThirdItemsCommissionScale = (decimal)reader["ThirdItemsCommissionScale"];
		...
		public static DistributorsInfo PopulateDistributorInfo(IDataReader reader)
		{
			...
			else
			{
                ...
                if (reader["BannerURL"] != System.DBNull.Value)
                {
                    distributorsInfo.BannerURL = (string)reader["BannerURL"];
                }
