...
namespace Hidistro.SaleSystem.Vshop
{
	public static class ShoppingCartProcessor
	{
        ...
        /// 创建订奶购物车
        /// </summary>
        public static List<ShoppingCartInfo> GetListShoppingCart(DateTime startSendDate,int quantityPerDay,int sendDays, string productSkuId, int buyAmount, int bargainDetialId = 0, int limitedTimeDiscountId = 0)
        {
            List<ShoppingCartInfo> list = new List<ShoppingCartInfo>();
            ShoppingCartInfo shoppingCartInfo = new ShoppingCartInfo();
            MemberInfo currentMember = MemberProcessor.GetCurrentMember();
            ...
            if (bargainDetialId > 0)
            {
                ...
                if (quantityPerDay > 0 && sendDays > 0)
                {
                    cartItemInfo = new ShoppingCartDao().GetCartItemInfo(startSendDate, quantityPerDay, sendDays, currentMember, productSkuId, buyAmount, 0, bargainDetialId, 0);
                }
                else
                {
                    cartItemInfo = new ShoppingCartDao().GetCartItemInfo(currentMember, productSkuId, buyAmount, 0, bargainDetialId, 0);
                }
            ...
            else
            {
                ...
                shoppingCartInfo.TemplateId = cartItemInfo.FreightTemplateId.ToString();
                shoppingCartInfo.Amount = decimal.Round(cartItemInfo.MilkSubTotal, 2); //牛奶配送总价
                shoppingCartInfo.Total = shoppingCartInfo.Amount;
                shoppingCartInfo.Exemption = 0m;
                shoppingCartInfo.ShipCost = 0m;
                shoppingCartInfo.GetPointNumber = cartItemInfo.PointNumber * cartItemInfo.Quantity;
