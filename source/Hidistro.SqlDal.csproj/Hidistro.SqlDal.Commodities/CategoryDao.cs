...
namespace Hidistro.SqlDal.Commodities
{
	public class CategoryDao
	{
		...
		public int CreateCategory(CategoryInfo category)
		{
            ...
            if (category.DelayDays > 0)
            {
                this.database.AddInParameter(storedProcCommand, "DelayDays", System.Data.DbType.Int32, category.DelayDays);
            }
            else
            {
                this.database.AddInParameter(storedProcCommand, "DelayDays", System.Data.DbType.Int32, 0);
            }
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("UPDATE Hishop_Categories SET [Name] = @Name, SKUPrefix = @SKUPrefix,AssociatedProductType = @AssociatedProductType, Meta_Title=@Meta_Title,Meta_Description = @Meta_Description, IconUrl = @IconUrl,Meta_Keywords = @Meta_Keywords, Notes1 = @Notes1, Notes2 = @Notes2, Notes3 = @Notes3,  Notes4 = @Notes4, Notes5 = @Notes5, RewriteName = @RewriteName,FirstCommission=@FirstCommission,SecondCommission=@SecondCommission,ThirdCommission=@ThirdCommission,DelayDays=@DelayDays WHERE CategoryId = @CategoryId;UPDATE Hishop_Categories SET FirstCommission=@FirstCommission,SecondCommission=@SecondCommission,ThirdCommission=@ThirdCommission WHERE Path like '%" + category.CategoryId + "|%'");
            ...
            this.database.AddInParameter(sqlStringCommand, "DelayDays", System.Data.DbType.Int32, category.DelayDays);
