...
namespace Hidistro.SqlDal.Commodities
{
	public class ProductDao
	{
		...
		public void UpdateShoppingCartsTemplateId(int productId, int templateId)
		{
			...
			{
				"update Hishop_ShoppingCarts set Templateid=",
				templateId,
				" where  SkuId like '",
				productId,
				"_%' and templateId<>",
		...
		public DbQueryResult GetProducts(ProductQuery query)
		{
			...
			{
				stringBuilder.AppendFormat(" AND datediff(dd,'{0}',AddedDate)>=0 ", DataHelper.GetSafeDateTimeFormat(query.StartDate.Value));
			}
			if (query.EndDate.HasValue)
			{
				stringBuilder.AppendFormat(" AND datediff(dd,'{0}',AddedDate)<=0", DataHelper.GetSafeDateTimeFormat(query.EndDate.Value));
			}
			...
			return DataHelper.PagingByRownumber(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "vw_Hishop_BrowseProductList p", "ProductId", stringBuilder.ToString(), "*");
		}
            ...
            if (str.Length > 1)
            {
                ...
                if ((dataTable == null ? false : dataTable.Rows.Count > 0))
                {
                    if (Resort)
                    {
                        ...
                        {
                            if (!nums.ContainsKey((int)row2["ProductId"]))
                            {
                                row2["Sort"] = 99999;
                            }
                            else
                            {
                                row2["Sort"] = nums[(int)row2["ProductId"]];
                            }
                        }
                    ...
                    {
                        productInfos1.Add(DataMapper.PopulateProduct(dataRow));
                    }
                }
                productInfos = productInfos1;
            }
            else
            {
                productInfos = productInfos1;
		...
		{
			string text = "UPDATE Hishop_SKUs  SET Stock = ";
			if (type == 2)
			{
				text += " Stock + ";
			}
			text += "@Stock WHERE ProductId = @ProductId";
			...
			this.database.AddInParameter(sqlStringCommand, "Stock", System.Data.DbType.Int32, quantity);
			this.database.AddInParameter(sqlStringCommand, "ProductId", System.Data.DbType.Int32, num_iid);
			this.database.AddInParameter(sqlStringCommand, "SkuId", System.Data.DbType.String, sku_id);
			return this.database.ExecuteNonQuery(sqlStringCommand);
		}
		...
		public product_item_model GetProductForApi(int num_iid)
		{
			...
			using (System.Data.IDataReader dataReader = this.database.ExecuteReader(sqlStringCommand))
			{
				if (dataReader.Read())
				{
					...
					product_item_model.cid = (int)dataReader["CategoryId"];
					if (dataReader["CategoryName"] != DBNull.Value)
					{
						product_item_model.cat_name = (string)dataReader["CategoryName"];
					}
					if (dataReader["BrandId"] != DBNull.Value)
					{
						product_item_model.brand_id = (int)dataReader["BrandId"];
					}
					if (dataReader["BrandName"] != DBNull.Value)
					{
						product_item_model.brand_name = (string)dataReader["BrandName"];
					}
					if (dataReader["TypeId"] != DBNull.Value)
					{
						product_item_model.type_id = (int)dataReader["TypeId"];
					}
					if (dataReader["TypeName"] != DBNull.Value)
					{
						product_item_model.type_name = (string)dataReader["TypeName"];
					}
					product_item_model.num_iid = (int)dataReader["ProductId"];
					if (dataReader["ProductCode"] != DBNull.Value)
					{
						product_item_model.outer_id = (string)dataReader["ProductCode"];
					}
					product_item_model.title = (string)dataReader["ProductName"];
					if (dataReader["ImageUrl1"] != DBNull.Value)
					{
						product_item_model.pic_url.Add(empty + (string)dataReader["ImageUrl1"]);
					}
					if (dataReader["ImageUrl2"] != DBNull.Value)
					{
						product_item_model.pic_url.Add(empty + (string)dataReader["ImageUrl2"]);
					}
					if (dataReader["ImageUrl3"] != DBNull.Value)
					{
						product_item_model.pic_url.Add(empty + (string)dataReader["ImageUrl3"]);
					}
					if (dataReader["ImageUrl4"] != DBNull.Value)
					{
						product_item_model.pic_url.Add(empty + (string)dataReader["ImageUrl4"]);
					}
					if (dataReader["ImageUrl5"] != DBNull.Value)
					{
						product_item_model.pic_url.Add(empty + (string)dataReader["ImageUrl5"]);
					}
					if (dataReader["Description"] != DBNull.Value)
					{
						product_item_model.desc = Globals.UrlEncode(((string)dataReader["Description"]).Replace(string.Format("src=\"{0}/Storage/master/gallery", Globals.ApplicationPath), string.Format("src=\"{0}/Storage/master/gallery", empty + Globals.ApplicationPath)).Replace('"', '“'));
					}
					product_item_model.list_time = new DateTime?((DateTime)dataReader["AddedDate"]);
					product_item_model.display_sequence = (int)dataReader["DisplaySequence"];
					ProductSaleStatus productSaleStatus = (ProductSaleStatus)dataReader["SaleStatus"];
					...
					product_item_model.sold_quantity = (int)dataReader["SaleCounts"];
				}
			}
		...
		public string GetPropsForApi(int num_iid)
		{
			...
			using (System.Data.IDataReader dataReader = this.database.ExecuteReader(sqlStringCommand))
			{
				while (dataReader.Read())
				{
					if (dictionary.Keys.Contains((string)dataReader["AttributeName"]))
					{
						Dictionary<string, string> dictionary2;
						string text;
						(dictionary2 = dictionary)[text = (string)dataReader["AttributeName"]] = dictionary2[text] + "," + (string)dataReader["ValueStr"];
					}
					else
					{
						dictionary.Add((string)dataReader["AttributeName"], (string)dataReader["ValueStr"]);
					}
				}
			}
		...
		public IList<product_sku_model> GetSkusForApi(int num_iid)
		{
			...
			using (System.Data.IDataReader reader = this.database.ExecuteReader(sqlStringCommand))
			{
				while (reader.Read())
				{
					product_sku_model product_sku_model = list.FirstOrDefault((product_sku_model item) => item.sku_id == (string)reader["SkuId"]);
					if (product_sku_model == null)
					{
						product_sku_model = new product_sku_model();
						product_sku_model.sku_id = (string)reader["SkuId"];
						if (reader["SKU"] != DBNull.Value)
						{
							product_sku_model.outer_sku_id = (string)reader["SKU"];
						}
						product_sku_model.quantity = (int)reader["Stock"];
						product_sku_model.price = (decimal)reader["SalePrice"];
						if (reader["AttributeName"] != DBNull.Value && reader["ValueStr"] != DBNull.Value)
						{
							product_sku_model.sku_properties_name = (string)reader["AttributeName"] + ":" + (string)reader["ValueStr"];
						}
						list.Add(product_sku_model);
					}
					else if (reader["AttributeName"] != DBNull.Value && reader["ValueStr"] != DBNull.Value)
					{
						product_sku_model expr_1DA = product_sku_model;
						string sku_properties_name = expr_1DA.sku_properties_name;
						expr_1DA.sku_properties_name = string.Concat(new string[]
						{
							sku_properties_name,
							";",
							(string)reader["AttributeName"],
							":",
							(string)reader["ValueStr"]
						});
					}
				}
			}
		...
		public int UpdateProductApproveStatusForApi(int num_iid, string approve_status)
		{
			...
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("UPDATE Hishop_Products SET SaleStatus = @SaleStatus  WHERE ProductId = @ProductId ");
			this.database.AddInParameter(sqlStringCommand, "SaleStatus", System.Data.DbType.Int32, (int)productSaleStatus);
			this.database.AddInParameter(sqlStringCommand, "ProductId", System.Data.DbType.Int32, num_iid);
