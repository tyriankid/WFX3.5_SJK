...
namespace Hidistro.SqlDal.Members
{
	public class DistributorsDao
	{
        ...
        public bool DeleteStreetInfo(string id)
        {
            string query = "DELETE FROM Site_Scope WHERE StreetId = '" + id+"'";
            System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
            return this.database.ExecuteNonQuery(sqlStringCommand) > 0;
        }
        

         public bool EditStreetName(string id, string streetName)
        {
            bool result;
         
                string text = "update   Site_Scope set StreetName='"+ streetName + "' where StreetId='"+ id + "'";
        ...
        public bool AddStreetInfo(string regionCode, string streetName, int UserId)
        {
            bool result;
            if (UserId <= 0)
            {
            ...
            {
                string text = "INSERT INTO Site_Scope(UserId,RegionCode,StreetName) VALUES(@UserId,@RegionCode,@StreetName)";
              
                System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(text);
               
                this.database.AddInParameter(sqlStringCommand, "UserId", System.Data.DbType.Int32, UserId);
                this.database.AddInParameter(sqlStringCommand, "RegionCode", System.Data.DbType.String, regionCode);
                this.database.AddInParameter(sqlStringCommand, "StreetName", System.Data.DbType.String, streetName);
        ...
        public bool CreateDistributor(DistributorsInfo distributor)
		{
			...
			{
				string text = "INSERT INTO aspnet_Distributors(UserId,StoreName,Logo,BackImage,RequestAccount,GradeId,ReferralUserId,ReferralPath, ReferralOrders,ReferralBlance, ReferralRequestBalance,ReferralStatus,StoreDescription,DistributorGradeId,SiteTel,SiteRegionId,SiteAddress,SiteStatus,SiteLat,SiteLng) VALUES(@UserId,@StoreName,@Logo,@BackImage,@RequestAccount,@GradeId,@ReferralUserId,@ReferralPath,@ReferralOrders,@ReferralBlance, @ReferralRequestBalance, @ReferralStatus,@StoreDescription,@DistributorGradeId,@SiteTel,@SiteRegionId,@SiteAddress,@SiteStatus,@SiteLat,@SiteLng)";
				if (!string.IsNullOrEmpty(distributor.CellPhone))
				{
					text = "UPDATE aspnet_Members set CellPhone=@CellPhone where UserId=@UserId;" + text;
				}
				System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(text);
                ...
                this.database.AddInParameter(sqlStringCommand, "SiteTel", System.Data.DbType.String, distributor.SiteTel);
                this.database.AddInParameter(sqlStringCommand, "SiteRegionId", System.Data.DbType.Int64, distributor.SiteRegionId);
                this.database.AddInParameter(sqlStringCommand, "SiteAddress", System.Data.DbType.String, distributor.SiteAddress);
                this.database.AddInParameter(sqlStringCommand, "SiteStatus", System.Data.DbType.Int64, distributor.SiteStatus);
                this.database.AddInParameter(sqlStringCommand, "SiteLat", System.Data.DbType.Double, distributor.SiteLat);
                this.database.AddInParameter(sqlStringCommand, "SiteLng", System.Data.DbType.Double, distributor.SiteLng);
                result = (this.database.ExecuteNonQuery(sqlStringCommand) > 0);
			}
			return result;
		}
		...
		public string UpdateDistributorSuperior(int userid, int tosuperuserid)
		{
			...
			if (distributorInfo != null)
			{
				stringBuilder.Append(string.Concat(new object[]
				{
					...
					";分销商名称是：",
					distributorInfo.StoreName,
					";"
				}));
			}
			DistributorsInfo distributorInfo2 = this.GetDistributorInfo(tosuperuserid);
			if (distributorInfo2 != null)
			{
				string text = string.Empty;
				int referralUserId = distributorInfo2.ReferralUserId;
				...
				using (System.Data.Common.DbConnection dbConnection = database.CreateConnection())
				{
					...
					try
					{
						...
						System.Data.Common.DbCommand sqlStringCommand = database.GetSqlStringCommand(text2);
						database.ExecuteNonQuery(sqlStringCommand, dbTransaction);
						stringBuilder.Append(text2 + ";");
						string text3 = tosuperuserid + "|" + userid;
						...
						sqlStringCommand = database.GetSqlStringCommand(text2);
						database.ExecuteNonQuery(sqlStringCommand, dbTransaction);
						stringBuilder.Append(text2 + ";");
						text2 = string.Concat(new object[]
						{
						...
						Globals.Debuglog(stringBuilder.ToString(), "_DebuglogModifyDistributorRefer.txt");
						result = "1";
					}
					catch
					{
		...
		{
			int result = 0;
			string text = string.Format("SELECT COUNT(*) FROM aspnet_Distributors where ReferralPath LIKE '{0}|%' OR ReferralPath LIKE '%|{0}|%' OR ReferralPath LIKE '%|{0}' OR ReferralPath='{0}'", Globals.GetCurrentMemberUserId(false));
		...
		{
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.AppendFormat("Select (SELECT count(UserId) FROM vw_Hishop_DistributorsMembers where  (ReferralPath='{0}' or ReferralPath like '%|{0}')) firstV,(SELECT count(UserId) FROM  vw_Hishop_DistributorsMembers where  ReferralPath like '{0}|%') secondV ,( select count(*) from aspnet_Members  s left join aspnet_Distributors d on s.userid=d.userid  where  s.Status in(1,9) and s.ReferralUserId={0} and d.StoreName is null) memberCount", topUserId);
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(stringBuilder.ToString());
			System.Data.DataTable result;
			using (System.Data.IDataReader dataReader = this.database.ExecuteReader(sqlStringCommand))
			{
		...
		public DbQueryResult GetDistributors(DistributorsQuery query, string TopUserId = null, string level = null)
		{
			...
			if (!string.IsNullOrEmpty(query.MicroSignal))
			{
				...
				stringBuilder.AppendFormat("UserBindName LIKE '%{0}%'", DataHelper.CleanSearchString(query.MicroSignal));
			}
        ...
        public DbQueryResult GetScopeListByUserId(StreetInfoQuery query)
        {
            StringBuilder stringBuilder = new StringBuilder(" 1=1");

            stringBuilder.Append(" AND UserId = " + query.distributorId);
            ...
            if (!string.IsNullOrEmpty(query.streetName) )
            {
                stringBuilder.Append(" AND StreetName like '%"+ query.streetName + "%' ");
            }
           
            return DataHelper.PagingByRownumber(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "Site_Scope", "StreetId", (stringBuilder.Length > 0) ? stringBuilder.ToString() : null, "*");
        }

        public DbQueryResult GetMilkCards(MilkCardQuery query)
        {
            StringBuilder stringBuilder = new StringBuilder(" 1=1");
            if (query.siteId > 0)
            {
                stringBuilder.Append(" AND sm.SiteId = " + query.siteId);
            }
            stringBuilder.Append(" AND sm.status = " + query.status);
            return DataHelper.PagingByRownumber(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "site_milkcard sm left join hishop_products hp on sm.productid=hp.productid  left join aspnet_members am on sm.userid = am.userid", "ID", (stringBuilder.Length > 0) ? stringBuilder.ToString() : null, "sm.*,hp.imageUrl1,hp.productname,am.username");
        }

        public MilkCardInfo GetMilkCard(Guid cardid)
        {
            string sql = "Select * from site_milkcard Where ID = '"+cardid+"' ";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(sql);
            using (System.Data.IDataReader dataReader = this.database.ExecuteReader(sqlStringCommand))
            {
                return ReaderConvert.ReaderToModel<MilkCardInfo>(dataReader);
            }
        }

        public MilkCardInfo GetMilkCard(string cardnum,string cardpwd)
        {
            string sql = "select sm.*, dateadd(day,hc.delaydays,getdate())as startSendDate from site_milkcard sm left join hishop_products hp on sm.productid = hp.productid left join hishop_categories hc on hp.categoryid = hc.categoryid where CardNum = '" + cardnum + "' and CardPassword = '"+cardpwd+"' ";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(sql);
            using (System.Data.IDataReader dataReader = this.database.ExecuteReader(sqlStringCommand))
            {
                return ReaderConvert.ReaderToModel<MilkCardInfo>(dataReader);
        ...
        public bool UpdateMilkCard(MilkCardInfo mcinfo)
        {
            string query = "update site_milkcard set ProductId=@ProductId,FreeSendDays=@FreeSendDays,FreeQuantityPerDay=@FreeQuantityPerDay,Status=@Status,OrderDate=@OrderDate,OrderId=@OrderId,UserId=@UserId where ID=@ID";
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
            this.database.AddInParameter(sqlStringCommand, "ID", System.Data.DbType.Guid, mcinfo.ID);
            this.database.AddInParameter(sqlStringCommand, "ProductId", System.Data.DbType.Int32, mcinfo.ProductId);
            this.database.AddInParameter(sqlStringCommand, "FreeSendDays", System.Data.DbType.Int32, mcinfo.FreeSendDays);
            this.database.AddInParameter(sqlStringCommand, "FreeQuantityPerDay", System.Data.DbType.Int32, mcinfo.FreeQuantityPerDay);
            this.database.AddInParameter(sqlStringCommand, "Status", System.Data.DbType.Int32, mcinfo.Status);
            this.database.AddInParameter(sqlStringCommand, "OrderDate", System.Data.DbType.DateTime, mcinfo.OrderDate);
            this.database.AddInParameter(sqlStringCommand, "OrderId", System.Data.DbType.String, mcinfo.OrderId);
            this.database.AddInParameter(sqlStringCommand, "UserId", System.Data.DbType.Int32, mcinfo.UserId);
            int num = this.database.ExecuteNonQuery(sqlStringCommand);
            return num > 0;
        }

        public bool CreateMilkCards(MilkCardInfo mc,int cardCount)
        {
            StringBuilder stringBuilder = new StringBuilder();

            for(int i = 0; i < cardCount; i++)
            {
                mc.CardNum = Globals.GetGenerateMilkCardNum();
                mc.CardPassword = Guid.NewGuid().ToString("N").Substring(0, 8);
                stringBuilder.AppendLine("INSERT INTO site_milkcard").AppendLine("(ID,CardNum,CardPassword,SiteId,Status,CreateDate,ProductId,FreeSendDays,FreeQuantityPerDay)")
                 .AppendLine(string.Format("VALUES(newid(),'{0}','{1}',{2},{3},'{4}',{5},{6},{7});", mc.CardNum, mc.CardPassword, mc.SiteId, mc.Status, mc.CreateDate, mc.ProductId, mc.FreeSendDays, mc.FreeQuantityPerDay));

            }

            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(stringBuilder.ToString());

            int num = int.Parse(this.database.ExecuteNonQuery(sqlStringCommand).ToString());
            bool flag = num == cardCount;
            return flag;
        }

        public int SendMilkCards(string[] openids,string[] cardids)
        {
            StringBuilder stringBuilder = new StringBuilder();

            for (int i = 0; i < cardids.Length && i < openids.Length; i++){
                stringBuilder.Append(string.Format("update site_milkcard set userid = (select userid from aspnet_members where openid = '{0}') where id = '{1}' ;", openids[i],cardids[i]));
            }
        ...
        public int GetMilkCardCount(int userid)
        {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.Append(string.Format("select count(*) from site_milkcard where userid = {0} and status = 0", userid));
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(stringBuilder.ToString());
            return Globals.ToNum(this.database.ExecuteScalar(sqlStringCommand));
        }

        public DataTable GetMilkCardList(int userid)
        {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.Append(string.Format("select sm.*, dateadd(day,hc.delaydays,getdate())as startSendDate from site_milkcard sm left join hishop_products hp on sm.productid = hp.productid left join hishop_categories hc on hp.categoryid = hc.categoryid where userid = {0} and status = 0", userid));
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(stringBuilder.ToString());
            return this.database.ExecuteDataSet(sqlStringCommand).Tables[0];
        }

        public DbQueryResult GetQuestList(QuestInfoQuery query)
        {
            StringBuilder stringBuilder = new StringBuilder(" 1=1");

            stringBuilder.Append(" AND a.SiteId = " + query.UserId);

            if (!string.IsNullOrEmpty(query.OrderId))
            {
                stringBuilder.Append(" AND  a.OrderId = '" + query.OrderId + "' ");
            }
            if (!string.IsNullOrEmpty(query.UserName))
            {
                stringBuilder.Append(" AND RealName = '" + query.UserName + "' ");
            }

            if (!string.IsNullOrEmpty(query.SendStartDate))
            {
                stringBuilder.Append(" AND QuestDate >= '" + query.SendStartDate + "' ");
            }
            if (!string.IsNullOrEmpty(query.SendEndDate))
            {
                stringBuilder.Append(" AND QuestDate <= '" + query.SendEndDate + "' ");
            }
            if (!string.IsNullOrEmpty(query.Status))
            {
                stringBuilder.Append(" AND Status = " + query.Status + " ");
            }

            return DataHelper.PagingByRownumber(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "Site_QuestList a inner join Hishop_Orders b on a.OrderId=b.OrderId inner join Hishop_Products c on c.ProductId=a.ProductId ", "QuestId", (stringBuilder.Length > 0) ? stringBuilder.ToString() : null, "a.*,b.Address,b.ShippingRegion,b.RealName,b.CellPhone,c.ProductName");
        }

        public string GetQuestCount(QuestInfoQuery query)
        {
          
            string sqlQuery = "( select  a.*,b.Address,b.ShippingRegion,b.RealName,b.CellPhone,c.ProductName from Site_QuestList a inner join Hishop_Orders b on a.OrderId=b.OrderId inner join Hishop_Products c on c.ProductId=a.ProductId where ";
            ...
            string sql1 = "select  count(*) from "+t +" where Status=0" ;
            string sql2 = "select  count(*) from " + t + " where Status=1";
            

            
            System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(sql1);
            object  dataSet1 = this.database.ExecuteScalar(sqlStringCommand);
            sqlStringCommand = this.database.GetSqlStringCommand(sql2);
            object dataSet2 = this.database.ExecuteScalar(sqlStringCommand);
            return dataSet1.ToString() + "," + dataSet2.ToString() ;

        }

        

        public  bool ChangStatus(string  ids,int opid)
        {
            bool result;
            string query = "UPDATE Site_QuestList set OperaterTime=getdate(),Status=1,OperaterId=" + opid+"  WHERE QuestId in (" + ids + ") ";
            System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
            result = (this.database.ExecuteNonQuery(sqlStringCommand) > 0);
            return result;
        }
        ...
        public DataTable GetScopeListBySiteId(int siteId)
        {
            string query2 = "Select * from site_scope Where userid = (select userid from aspnet_managers where siteid =" + siteId.ToString()+" )";
		...
		public System.Data.DataTable GetDownDistributors(DistributorsQuery query, out int total, string sort, string order)
		{
			StringBuilder stringBuilder = new StringBuilder(" 1=1 ");
			string text = "";
			if (query.GradeId > 0)
			...
			string query3 = string.Concat(new object[]
			{
				...
				") as w ) AS M ) AS K  WHERE  RowIndex between ",
				num,
				" and ",
				num2
			});
		...
		public BalanceDrawRequestInfo GetBalanceDrawRequestById(string SerialID)
		{
			...
			else
			{
				...
				if (balanceDrawRequestInfo != null && (balanceDrawRequestInfo.RequestType == 3 || balanceDrawRequestInfo.RequestType == 0) && string.IsNullOrEmpty(balanceDrawRequestInfo.RedpackId))
				{
					...
					if (num > 0)
					{
						balanceDrawRequestInfo.RedpackId = text;
					}
				}
		...
		public bool SetBalanceDrawRequestIsCheckStatus(int[] serialid, int checkValue, string Remark = null, string Amount = null)
		{
			...
			text = text + " where IsCheck not in(-1,2) and  SerialID in(" + string.Join<int>(",", serialid) + ")";
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(text);
        ...
        public bool EditDisbutosSiteInfos(DistributorsInfo entity)
        {
            string query = "UPDATE aspnet_Distributors set StoreName=@StoreName,SiteTel=@SiteTel,SiteRegionId=@SiteRegionId,SiteAddress=@SiteAddress,StoreDescription=@StoreDescription WHERE UserId=@UserId";
            System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
            this.database.AddInParameter(sqlStringCommand, "UserId", System.Data.DbType.String, entity.UserId);
            this.database.AddInParameter(sqlStringCommand, "StoreName", System.Data.DbType.String, entity.StoreName);
            this.database.AddInParameter(sqlStringCommand, "SiteTel", System.Data.DbType.String, entity.SiteTel);
            this.database.AddInParameter(sqlStringCommand, "SiteRegionId", System.Data.DbType.Int32, entity.SiteRegionId);
            this.database.AddInParameter(sqlStringCommand, "SiteAddress", System.Data.DbType.String, entity.SiteAddress);
            this.database.AddInParameter(sqlStringCommand, "StoreDescription", System.Data.DbType.String, entity.StoreDescription);
            return this.database.ExecuteNonQuery(sqlStringCommand) > 0;
        }
        public bool EditDisbutosBannerURL(DistributorsInfo entity)
        {
            string query = "UPDATE aspnet_Distributors set BannerURL=@BannerURL WHERE UserId=@UserId";
            System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
            this.database.AddInParameter(sqlStringCommand, "UserId", System.Data.DbType.String, entity.UserId);
            this.database.AddInParameter(sqlStringCommand, "BannerURL", System.Data.DbType.String, entity.BannerURL);
            return this.database.ExecuteNonQuery(sqlStringCommand) > 0;
        }
