	...
	public class IntegralDetailDao
	{
		...
		public DbQueryResult GetIntegralDetail(IntegralDetailQuery query)
		{
			...
			if (query.StartTime.HasValue)
			{
				...
				stringBuilder.AppendFormat("  datediff(dd,'{0}',TrateTime) >=0 ", DataHelper.GetSafeDateTimeFormat(query.StartTime.Value));
			}
			if (query.EndTime.HasValue)
			{
				...
				stringBuilder.AppendFormat(" datediff(dd,'{0}',TrateTime)<=0 ", DataHelper.GetSafeDateTimeFormat(query.EndTime.Value));
			}
			if (query.UserId > 0)
			{
				if (stringBuilder.Length > 0)
