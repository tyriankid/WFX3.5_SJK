...
namespace Hidistro.SqlDal.Members
{
	public class MemberDao
	{
		...
		public DbQueryResult GetMembers(MemberQuery query, bool isNotBindUserName = false)
		{
			...
			if (query.Stutas.HasValue)
			{
				if (Convert.ToInt32(query.Stutas) > 0)
				{
					...
					if (query.Stutas == UserStatus.Visitor)
					{
						stringBuilder.AppendFormat(" PayOrderDate is null and (Status=1 or status=9) ", new object[0]);
					}
					else
					{
			...
			if (!string.IsNullOrEmpty(query.StoreName))
			{
				...
				{
					stringBuilder.AppendFormat(" ReferralUserId not in  (select userid from  dbo.aspnet_Distributors where  UserId=m.ReferralUserId)", new object[0]);
				}
				else
				{
					stringBuilder.AppendFormat(" ReferralUserId in  (SELECT UserId  FROM aspnet_Distributors where storename='{0}' )", query.StoreName);
				}
			}
		...
		public System.Data.DataTable GetMembersByUserId(int referralUserId, int pageIndex, int pageSize, out int total, string sort, string order)
		{
			total = 0;
			...
			stringBuilder.AppendFormat("select ROW_NUMBER()  OVER (ORDER BY {0} {1}) as [RowIndex],* From (", sort, order);
			stringBuilder.AppendFormat("SELECT * FROM (", new object[0]);
			stringBuilder.AppendFormat("select s.userid,username", new object[0]);
			stringBuilder.AppendFormat(",(SELECT Name FROM aspnet_MemberGrades WHERE GradeId = s.GradeId) AS GradeName", new object[0]);
			stringBuilder.AppendFormat(",UserHead,CreateDate,s.ReferralUserId,d.StoreName", new object[0]);
			stringBuilder.AppendFormat(", (select count(*) from Hishop_Orders where UserId=s.UserId and OrderStatus<>4 and PayDate is not null ) as OrderMumber", new object[0]);
			stringBuilder.AppendFormat(", LastOrderDate as OrderDate", new object[0]);
			stringBuilder.AppendFormat(", (select SUM(OrderTotal) from Hishop_Orders where UserId=s.UserId and OrderStatus<>4 and PayDate is not null ) as OrdersTotal ", new object[0]);
			stringBuilder.AppendFormat("from aspnet_Members s left join aspnet_Distributors d on s.userid=d.userid ", new object[0]);
			stringBuilder.AppendFormat("where  s.ReferralUserId={0} and Status in(1,9) and d.StoreName is null ", referralUserId);
			stringBuilder.AppendFormat(")  AS W ", new object[0]);
			stringBuilder.AppendFormat(") AS M ", new object[0]);
			stringBuilder.AppendFormat(") AS K  WHERE  RowIndex between {0} and {1}", num, num2);
			string query2 = stringBuilder.ToString();
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("SELECT * FROM aspnet_Members WHERE SessionId = @SessionId");
		...
		{
			string text = "INSERT INTO aspnet_Members(GradeId,ReferralUserId,UserName,CreateDate,OrderNumber,Expenditure,Points,TopRegionId, RegionId,OpenId, SessionId, SessionEndTime,Password,UserHead,UserBindName,Status,AlipayUserId,AlipayUsername,AlipayOpenid,AlipayLoginId,AlipayAvatar) VALUES(@GradeId,@ReferralUserId,@UserName,@CreateDate,0,0,0,0,0,@OpenId, @SessionId, @SessionEndTime,@Password,@UserHead,@UserBindName,@Status,@AlipayUserId,@AlipayUsername,@AlipayOpenid,@AlipayLoginId,@AlipayAvatar)";
			if (member.OpenId != null && member.OpenId.Trim() != "")
			{
				text = "if not exists(select top 1 userid from aspnet_Members where openid='" + member.OpenId.Trim() + "') " + text;
			}
			...
			int num = Convert.ToInt32(UserStatus.Normal);
			if (member.Status == Convert.ToInt32(UserStatus.Visitor))
			{
				num = member.Status;
			}
			this.database.AddInParameter(sqlStringCommand, "Status", System.Data.DbType.Int32, num);
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("UPDATE aspnet_Members SET Points = Points+@Points  WHERE UserId = @UserId");
			this.database.AddInParameter(sqlStringCommand, "Points", System.Data.DbType.Int32, point);
			this.database.AddInParameter(sqlStringCommand, "UserId", System.Data.DbType.Int32, userid);
			return this.database.ExecuteNonQuery(sqlStringCommand) > 0;
		}
		...
		public bool SetUserHeadAndUserName(string OpenId, string UserHead, string UserName, int IsAuthorizeWeiXin = 1)
		{
			...
			{
			case -1:
				text = "UPDATE  aspnet_Members SET IsAuthorizeWeiXin=0 where OpenId = @OpenId ";
				break;
			case 0:
				text = "UPDATE  aspnet_Members SET UserName = @UserName,UserHead=@UserHead where OpenId = @OpenId ";
				break;
			case 1:
				text = "UPDATE  aspnet_Members SET UserName = @UserName,UserHead=@UserHead,IsAuthorizeWeiXin=1 where OpenId = @OpenId ";
				break;
			case 2:
				text = "UPDATE  aspnet_Members SET IsAuthorizeWeiXin=1 where OpenId = @OpenId ";
				break;
			}
			bool result;
			if (!string.IsNullOrEmpty(text))
			{
				...
				text = "UPDATE  aspnet_Members SET Status=1 where OpenId = @OpenId and Status=@Status";
				sqlStringCommand = this.database.GetSqlStringCommand(text);
				this.database.AddInParameter(sqlStringCommand, "OpenId", System.Data.DbType.String, OpenId);
				this.database.AddInParameter(sqlStringCommand, "Status", System.Data.DbType.Int32, Convert.ToInt32(UserStatus.Visitor));
		...
		{
			string text = "Update aspnet_Members set OpenId='',IsAuthorizeWeiXin=0 ";
			if (from == "fuwu")
			{
				text = "Update aspnet_Members set AlipayOpenid=''";
			}
			else
			{
				text += ";delete from WeiXin_RecentOpenID";
			}
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(text);
