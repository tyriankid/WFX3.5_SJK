...
namespace Hidistro.SqlDal.Orders
{
	public class OrderDao
	{
		...
		public System.Data.DataSet GetTradeOrders(OrderQuery query, int distributorUserId, out int records)
		{
			...
			this.database.AddInParameter(storedProcCommand, "sqlPopulate", System.Data.DbType.String, this.BuildOrdersQuery(query, new int?(distributorUserId)));
		...
		private string BuildOrdersQuery(OrderQuery query, int? distributorUserId)
		{
			...
			stringBuilder.Append("SELECT OrderId FROM Hishop_Orders WHERE 1=1 ");
			...
			{
				stringBuilder.AppendFormat(" and orderDate is not null AND datediff(ss,'{0}',orderDate)>=0", DataHelper.GetSafeDateTimeFormat(query.StartDate.Value));
			}
			if (query.EndDate.HasValue)
			{
				stringBuilder.AppendFormat(" and orderDate is not null AND datediff(ss,'{0}',orderDate)<=0", DataHelper.GetSafeDateTimeFormat(query.EndDate.Value));
			}
			...
			else if (query.Status != OrderStatus.All)
			{
				...
				{
					stringBuilder.AppendFormat(" AND (OrderStatus = {0} OR (OrderStatus = {1} AND Gateway = 'hishop.plugins.payment.podrequest'))", 2, 1);
				}
				else
				{
		...
		public System.Data.DataSet GetAutoBatchOrdersIdList()
		{
			...
			if (masterSettings.CloseOrderDays > 0)
			{
				...
				{
					"select OrderID from Hishop_Orders where OrderStatus=",
					1.ToString(),
					" AND OrderDate <= @OrderDate AND Gateway<>'hishop.plugins.payment.podrequest';SELECT OrderId FROM  Hishop_Orders WHERE  OrderStatus=",
					3.ToString(),
					" AND ShippingDate <= '",
					DateTime.Now.AddDays((double)(-(double)masterSettings.FinishOrderDays)),
				...
				this.database.AddInParameter(sqlStringCommand, "OrderDate", System.Data.DbType.DateTime, DateTime.Now.AddDays((double)(-(double)masterSettings.CloseOrderDays)));
		...
		public DbQueryResult GetUserOrderByPage(int userId, OrderQuery query)
		{
			...
			text3 = text3 + " SELECT ID, OrderId, ThumbnailsUrl, ItemDescription, SKUContent, SKU,OrderItemsStatus, ProductId,Quantity,ReturnMoney,SkuID,ItemAdjustedPrice,Type,PointNumber,LimitedTimeDiscountId,DiscountAverage,SendStartDate,SendEndDate,QuantityPerDay,SendDays FROM Hishop_OrderItems WHERE OrderId IN  (" + text2 + ")   ";
		...
		public OrderInfo GetCalculadtionCommission(OrderInfo order, int isModifyOrders)
		{
			...
			if (distributorsInfo != null)
			{
				...
				System.Data.DataView dataView2 = new System.Data.DataView();
				if (isModifyOrders == 0)
				{
					dataView2 = distributorGradeDao.GetAllDistributorGrade().DefaultView;
					flag = (distributorsInfo.ReferralStatus == 0);
					if (distributorsInfo.DistriGradeId.ToString() != "0")
					{
						dataView2.RowFilter = " GradeId=" + distributorsInfo.DistriGradeId;
						if (dataView2.Count > 0)
						{
							d = decimal.Parse(dataView2[0]["FirstCommissionRise"].ToString());
						}
					}
					if (masterSettings.EnableCommission && !string.IsNullOrEmpty(distributorsInfo.ReferralPath) && distributorsInfo.ReferralPath != "0")
					{
						...
						if (array.Length == 1)
						{
							...
							{
								flag2 = (distributorInfo.ReferralStatus == 0);
								if (distributorInfo.DistriGradeId.ToString() != "0")
								{
									dataView2.RowFilter = " GradeId=" + distributorInfo.DistriGradeId;
									if (dataView2.Count > 0)
									{
										d2 = decimal.Parse(dataView2[0]["SecondCommissionRise"].ToString());
									}
								}
							}
						}
						else
						{
							...
							{
								flag3 = (distributorInfo2.ReferralStatus == 0);
								if (distributorInfo2.DistriGradeId.ToString() != "0")
								{
									dataView2.RowFilter = " GradeId=" + distributorInfo2.DistriGradeId;
									if (dataView2.Count > 0)
									{
										d3 = decimal.Parse(dataView2[0]["ThirdCommissionRise"].ToString());
									}
								}
							}
						}
					}
					dataView = new CategoryDao().GetCategories().DefaultView;
				}
				...
				foreach (KeyValuePair<string, LineItemInfo> current in lineItems)
				{
					...
					if (lineItemInfo.Type == 0)
					{
						...
						if (isModifyOrders == 0)
						{
							if (num > 0m)
							{
								...
								{
									lineItemInfo.ItemsCommissionScale = (flag ? ((lineItemInfo.FirstCommission == 0m) ? 0m : ((lineItemInfo.FirstCommission + d) / 100m)) : 0m);
									lineItemInfo.SecondItemsCommissionScale = (flag2 ? ((lineItemInfo.SecondCommission == 0m) ? 0m : ((lineItemInfo.SecondCommission + d2) / 100m)) : 0m);
									lineItemInfo.ThirdItemsCommissionScale = (flag3 ? ((lineItemInfo.ThirdCommission == 0m) ? 0m : ((lineItemInfo.ThirdCommission + d3) / 100m)) : 0m);
									lineItemInfo.ItemsCommission = (flag ? ((lineItemInfo.FirstCommission > 0m) ? (lineItemInfo.ItemsCommissionScale * num) : 0m) : 0m);
									lineItemInfo.SecondItemsCommission = (flag2 ? ((lineItemInfo.SecondCommission > 0m) ? (lineItemInfo.SecondItemsCommissionScale * num) : 0m) : 0m);
									lineItemInfo.ThirdItemsCommission = (flag3 ? ((lineItemInfo.ThirdCommission > 0m) ? (lineItemInfo.ThirdItemsCommissionScale * num) : 0m) : 0m);
								}
								else
								{
									...
									if (productCategories.Rows.Count > 0)
									{
										...
										{
											dataView.RowFilter = " CategoryId=" + productCategories.Rows[0][0];
											string text = dataView[0]["FirstCommission"].ToString();
											string text2 = dataView[0]["SecondCommission"].ToString();
											string text3 = dataView[0]["ThirdCommission"].ToString();
											if (!string.IsNullOrEmpty(text) && !string.IsNullOrEmpty(text2) && !string.IsNullOrEmpty(text3))
											{
												lineItemInfo.ItemsCommissionScale = (flag ? ((decimal.Parse(text) > 0m) ? ((decimal.Parse(text) + d) / 100m) : 0m) : 0m);
												lineItemInfo.SecondItemsCommissionScale = (flag2 ? ((decimal.Parse(text2) > 0m) ? ((decimal.Parse(text2) + d2) / 100m) : 0m) : 0m);
												lineItemInfo.ThirdItemsCommissionScale = (flag3 ? ((decimal.Parse(text3) > 0m) ? ((decimal.Parse(text3) + d3) / 100m) : 0m) : 0m);
												lineItemInfo.ItemsCommission = (flag ? ((decimal.Parse(text) > 0m) ? (lineItemInfo.ItemsCommissionScale * num) : 0m) : 0m);
												lineItemInfo.SecondItemsCommission = (flag2 ? ((decimal.Parse(text2) > 0m) ? (lineItemInfo.SecondItemsCommissionScale * num) : 0m) : 0m);
												lineItemInfo.ThirdItemsCommission = (flag3 ? ((decimal.Parse(text3) > 0m) ? (lineItemInfo.ThirdItemsCommissionScale * num) : 0m) : 0m);
											}
										}
									}
								}
						...
						lineItemInfo.ItemsCommission = num * lineItemInfo.ItemsCommissionScale * lineItemInfo.CommissionDiscount / 100m;
						lineItemInfo.SecondItemsCommission = num * lineItemInfo.SecondItemsCommissionScale * lineItemInfo.CommissionDiscount / 100m;
						lineItemInfo.ThirdItemsCommission = num * lineItemInfo.ThirdItemsCommissionScale * lineItemInfo.CommissionDiscount / 100m;
					}
					else
					{
						lineItemInfo.ItemsCommission = 0m;
					...
					lineItemInfo.ItemsCommission = Math.Round(lineItemInfo.ItemsCommission, 2);
					lineItemInfo.SecondItemsCommission = Math.Round(lineItemInfo.SecondItemsCommission, 2);
					lineItemInfo.ThirdItemsCommission = Math.Round(lineItemInfo.ThirdItemsCommission, 2);
				}
			}
			else
			{
		...
		public System.Data.DataSet GetDistributorOrder(OrderQuery query)
		{
			...
			string text2 = "SELECT UserID,OrderId, OrderDate,FinishDate, OrderStatus,PaymentTypeId, OrderTotal,Gateway,FirstCommission,SecondCommission,ThirdCommission FROM Hishop_Orders o WHERE ReferralUserId = @UserId";
			text2 += text;
			text2 += " ORDER BY OrderDate DESC";
		...
		public System.Data.DataSet GetDistributorOrderByDetials(OrderQuery query)
		{
			...
			string text2 = "SELECT UserId,OrderId, OrderDate,FinishDate, OrderStatus,PaymentTypeId, OrderTotal,Gateway,FirstCommission,SecondCommission,ThirdCommission FROM Hishop_Orders o WHERE OrderId in (select OrderId from Hishop_Commissions where UserId=@UserId and ReferralUserId=@ReferralUserId) ";
			text2 += text;
			text2 += " ORDER BY OrderDate DESC";
		...
		public DbQueryResult GetOrders(OrderQuery query)
		{
			...
			if (query.OrderItemsStatus.HasValue)
			{
				...
				else if (query.Status != OrderStatus.Closed || (query.OrderItemsStatus.Value != OrderStatus.Refunded && query.OrderItemsStatus.Value != OrderStatus.Returned))
				{
					stringBuilder.AppendFormat(" AND OrderId IN (SELECT OrderId FROM Hishop_OrderItems WHERE OrderItemsStatus={0})", (int)query.OrderItemsStatus.Value);
				}
			}
			...
			else if (query.Status == OrderStatus.Closed && query.OrderItemsStatus.HasValue && (query.OrderItemsStatus.Value == OrderStatus.Refunded || query.OrderItemsStatus.Value == OrderStatus.Returned))
			{
				stringBuilder.AppendFormat(" AND OrderStatus = {0} and OrderID in(select OrderID from Hishop_OrderReturns where  handlestatus=2)", (int)query.Status);
			}
			else if (query.Status != OrderStatus.All)
			{
                if (query.Status == OrderStatus.Finished)    //临时修改
                {
                    stringBuilder.AppendFormat(" AND OrderStatus in (5,2,3)");
                }
                else
                {
                    stringBuilder.AppendFormat(" AND OrderStatus = {0}", (int)query.Status);
            ...
            if (query.ReferralUserId != null)             //临时修改
            {
        ...
        public string GetOrderCountInfo(OrderQuery query)
        {
            StringBuilder stringBuilder = new StringBuilder("1=1");
            if (query.Type.HasValue)
            {
            ...
            string sql = "  select count(*) as count ,Convert(decimal(18,2),sum(OrderTotal))  as OrderTotal  from  vw_Hishop_Order where  " + stringBuilder.ToString();
            ...
            return dataSet.Tables[0].Rows[0][0] + "," + dataSet.Tables[0].Rows[0][1];
        }
        ...
        public DbQueryResult GetMemberDetailOrders(MemberDetailOrderQuery query)
		{
			...
			{
				stringBuilder.AppendFormat(" AND  ((OrderStatus in (" + string.Join(",", query.Status.Select(delegate(OrderStatus t)
				{
					int num = (int)t;
					return num.ToString();
				}).ToArray<string>()) + ")) ", new object[0]);
				if (query.Status.Contains(OrderStatus.BuyerAlreadyPaid))
				{
					stringBuilder.AppendFormat(" OR (OrderStatus = 1 AND Gateway = 'hishop.plugins.payment.podrequest')", new object[0]);
				}
			...
			{
				stringBuilder.AppendFormat(" AND datediff(dd,'{0}',FinishDate)>=0", DataHelper.GetSafeDateTimeFormat(query.StartFinishDate.Value));
			}
			if (query.EndFinishDate.HasValue)
			{
				stringBuilder.AppendFormat(" AND datediff(dd,'{0}',FinishDate)<=0", DataHelper.GetSafeDateTimeFormat(query.EndFinishDate.Value));
			}
			return DataHelper.PagingByRownumber(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "vw_Hishop_Order", "OrderId", stringBuilder.ToString(), "*");
		}
		...
		public System.Data.DataSet GetOrdersByOrderIDList(string orderIds)
		{
			...
			query = string.Format(string.Concat(new string[]
			{
				"with v as (SELECT ",
				text,
				", row_number() over (partition by ShipTo+CONVERT(VARCHAR(11), RegionId)+[Address]+CellPhone order by  RegionId desc) as rownumber from Hishop_Orders where   OrderId in ({0})) select ",
				text,
		...
		public void UpdateOrderItemBalance(string orderid)
		{
			...
			if (orderInfo != null)
			{
				decimal num = orderInfo.BalancePayMoneyTotal;
				...
				foreach (LineItemInfo current in orderInfo.LineItems.Values)
				{
					if (!flag)
					{
						if (current.ItemAdjustedPrice - current.ItemAdjustedCommssion <= num)
						{
							num -= current.ItemAdjustedPrice - current.ItemAdjustedCommssion;
							current.BalancePayMoney = current.ItemAdjustedPrice - current.ItemAdjustedCommssion;
						}
						else
						{
							current.BalancePayMoney = num;
							num = 0m;
						}
						if (num <= 0m)
						{
					...
					new LineItemDao().UpdateBalancePayMoney(current.ID, current.BalancePayMoney, current.OrderItemsStatus, null);
				}
				if (num > 0m)
				{
					orderInfo.BalancePayFreightMoneyTotal = num;
				}
			}
        ...
        public bool CreatOrder(OrderInfo orderInfo, System.Data.Common.DbTransaction dbTran)
		{
			...
			this.database.AddInParameter(storedProcCommand, "CouponFreightMoneyTotal", System.Data.DbType.Decimal, orderInfo.CouponFreightMoneyTotal);
			this.database.AddInParameter(storedProcCommand, "LogisticsTools", System.Data.DbType.Int32, (int)orderInfo.logisticsTools);
		...
		public bool UpdateOrder(OrderInfo order, System.Data.Common.DbTransaction dbTran = null)
		{
			...
			decimal num = Globals.GetPoint(total - order.AdjustedFreight);
			this.database.AddInParameter(sqlStringCommand, "BalancePayFreightMoneyTotal", System.Data.DbType.Decimal, balancePayFreightMoneyTotal);
			this.database.AddInParameter(sqlStringCommand, "BalancePayMoneyTotal", System.Data.DbType.Decimal, balancePayMoneyTotal);
		...
		public bool UpdateOrderItems(OrderInfo order)
		{
			...
			foreach (LineItemInfo current in order.LineItems.Values)
			{
				...
				{
					obj,
					"Update Hishop_OrderItems Set BalancePayMoney='",
					current.BalancePayMoney,
					"' Where Id ='",
					current.ID,
					"';"
				});
			}
			Globals.Debuglog("订单详细！！！" + text, "_DebuglogOrder.txt");
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(text);
		...
		public System.Data.DataTable GetAllOrderID(int siteid)
		{


            string query = "select OrderId,IsPrinted,OrderStatus,Gateway from Hishop_Orders  with (nolock)   where 1=1 ";
                if(siteid>0)
                query+=" AND ReferralUserId=" + siteid ;
		...
		public int GetCountOrderIDByStatus(OrderStatus? orderstatus, OrderStatus? itemstatus)
		{
			...
			{
				if (orderstatus.Value != OrderStatus.Closed || !itemstatus.HasValue || (itemstatus.Value != OrderStatus.Refunded && itemstatus.Value != OrderStatus.Returned))
				{
			...
			if (itemstatus.HasValue)
			{
				...
				if ((orderstatus.HasValue && orderstatus.Value == OrderStatus.Closed && itemstatus.Value == OrderStatus.Refunded) || itemstatus.Value == OrderStatus.Returned)
				{
					object obj = text;
					text = string.Concat(new object[]
					{
						obj,
						" OrderStatus=",
						(int)orderstatus.Value,
						" and OrderId in(SELECT OrderId FROM Hishop_OrderReturns where handlestatus=2)"
					});
				}
				else
				{
		...
		public System.Data.DataTable GetOrderMarkingAllOrderID(string OrderMarking, bool isWaitPay)
		{
