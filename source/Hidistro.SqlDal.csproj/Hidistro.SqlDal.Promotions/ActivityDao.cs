...
namespace Hidistro.SqlDal.Promotions
{
	public class ActivityDao
	{
		...
		public System.Data.DataTable QuerySelProducts()
		{
			StringBuilder stringBuilder = new StringBuilder();
			stringBuilder.AppendFormat("select * from Hishop_Activities_Product where ActivitiesId in(select ActivitiesId from Hishop_Activities where EndTime>getdate())", new object[0]);
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(stringBuilder.ToString());
			System.Data.DataTable result;
			using (System.Data.IDataReader dataReader = this.database.ExecuteReader(sqlStringCommand))
			{
