...
namespace Hidistro.SqlDal.Promotions
{
	public class GameDao
	{
		...
		public bool Create(GameInfo model, out int gameId)
		{
			...
			stringBuilder.Append("if not exists(select top 1 gameid from Hishop_PromotionGame where KeyWork=@KeyWork) INSERT INTO [Hishop_PromotionGame]\r\n                       ([GameType]\r\n                       ,[GameTitle]\r\n                       ,[Description]\r\n                       ,[BeginTime]\r\n                       ,[EndTime]\r\n                       ,[ApplyMembers]\r\n                       ,[DefualtGroup]\r\n                       ,[CustomGroup]\r\n                       ,[NeedPoint]\r\n                       ,[GivePoint]\r\n                       ,[OnlyGiveNotPrizeMember]\r\n                       ,[PlayType]\r\n                       ,[NotPrzeDescription]\r\n                       ,[GameUrl]\r\n                       ,[GameQRCodeAddress]\r\n                       ,[Status],[KeyWork]\r\n                       ,[LimitEveryDay]\r\n                       ,[MaximumDailyLimit]\r\n                       ,[PrizeRate]\r\n                       ,[MemberCheck])\r\n                        VALUES ");
		...
		public bool GetPrizesDeliveInfo(PrizesDeliveQuery query, out string GameTitle, out int UserId, out string prizeName)
		{
			...
			UserId = 0;
			prizeName = "";
			System.Data.Common.DbCommand sqlStringCommand = this._database.GetSqlStringCommand("select top 1 GameTitle, UserId,ProductName from [vw_Hishop_PrizesDeliveryRecord]   where LogId=" + query.LogId.ToString() + " and  ID=" + query.Id.ToString());
			...
			if (dataTable != null && dataTable.Rows.Count > 0)
			{
				...
				prizeName = Convert.ToString(dataTable.Rows[0]["ProductName"]);
				result = true;
			}
			else
			{
		...
		public bool DeletePrizesDelivery(int[] ids)
		{
			...
			{
				"delete from Hishop_PromotionGameResultMembersLog where LogId in(",
				string.Join<int>(",", ids),
				");delete from Hishop_PrizesDeliveryRecord where LogId in(",
				string.Join<int>(",", ids),
				")"
			});
			System.Data.Common.DbCommand sqlStringCommand = this._database.GetSqlStringCommand(query);
		...
		public System.Data.DataSet GetPrizeListByLogIDList(string orderIds, string pidlist)
		{
			...
			if (orderIds == "''")
			{
				query = string.Format(string.Concat(new string[]
				{
					"with v as (SELECT ",
					text,
					", row_number() over (partition by Receiver+ReggionPath+ExpressName+[Address]+Tel order by  ReggionPath desc) as rownumber from vw_Vshop_ProductPrizeLIst_WithDelievelyInfo where  ( LogId=0 AND Pid in ({1}) )) select ",
					text,
					",rownumber from v"
				}), orderIds, pidlist);
			}
			else
			{
				query = string.Format(string.Concat(new string[]
				{
					"with v as (SELECT ",
					text,
					", row_number() over (partition by Receiver+ReggionPath+ExpressName+[Address]+Tel order by  ReggionPath desc) as rownumber from vw_Vshop_ProductPrizeLIst_WithDelievelyInfo where   LogId in ({0}) OR ( LogId=0 AND Pid in ({1}) )) select ",
					text,
					",rownumber from v"
				}), orderIds, pidlist);
			}
		...
		public bool SetPrintOrderExpress(string orderId, string pId, string expressCompanyName, string expressCompanyAbb, string shipOrderNumber)
		{
			...
			Database database = DatabaseFactory.CreateDatabase();
			System.Data.Common.DbCommand sqlStringCommand = database.GetSqlStringCommand(query);
			database.AddInParameter(sqlStringCommand, "LogID", System.Data.DbType.String, orderId);
			database.AddInParameter(sqlStringCommand, "Pid", System.Data.DbType.String, pId);
			database.AddInParameter(sqlStringCommand, "CourierNumber", System.Data.DbType.String, shipOrderNumber);
			database.AddInParameter(sqlStringCommand, "ExpressName", System.Data.DbType.String, expressCompanyName);
			return database.ExecuteNonQuery(sqlStringCommand) > 0;
		}

		public System.Data.DataSet GetOrdersAndLines(string orderIds, string pIds)
		{
			if (!string.IsNullOrEmpty(orderIds) && !orderIds.Contains("'"))
			{
				orderIds = "'" + orderIds.Replace(",", "','") + "'";
			...
			if (orderIds == "''")
			{
				stringBuilder.AppendFormat("SELECT * FROM vw_Vshop_ProductPrizeLIst_WithDelievelyInfo WHERE (LogId=0 AND Pid IN ({1})) order by  CourierNumber asc,DeliveryTime desc ", orderIds, pIds);
			}
			else
			{
				stringBuilder.AppendFormat("SELECT * FROM vw_Vshop_ProductPrizeLIst_WithDelievelyInfo WHERE  LogId IN ({0}) OR (LogId=0 AND Pid IN ({1})) order by  CourierNumber asc,DeliveryTime desc ", orderIds, pIds);
			}
			Globals.Debuglog("发货单查询语句：" + stringBuilder.ToString(), "_SqlDebuglog.txt");
