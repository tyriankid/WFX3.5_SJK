...
namespace Hidistro.SqlDal.Store
{
	public class RoleDao
	{
		...
		{
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand("if( select count(*) from aspnet_Managers where RoleId = @RoleId ) = 0 BEGIN  DELETE FROM aspnet_RolePermissions WHERE RoleId = @RoleId; DELETE FROM aspnet_Roles WHERE RoleId = @RoleId END");
