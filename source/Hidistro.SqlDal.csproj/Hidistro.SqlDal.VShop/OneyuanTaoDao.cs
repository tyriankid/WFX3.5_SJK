	...
	public class OneyuanTaoDao
	{
		...
		public bool AddInitParticipantInfo(int creatNum = 60)
		{
            ...
            for (int i = 0; i < creatNum; i++)
            {
                ...
                dateTime = dateTime.AddHours((double)random.Next(72));
                dateTime = dateTime.AddMilliseconds((double)random.Next(999));
                while (dateTimes.Contains(dateTime))
                {
                    dateTime = dateTime.AddMilliseconds((double)random.Next(999));
                }
            ...
            foreach (DateTime dateTime2 in dateTimes)
            {
                ...
                this.database.AddInParameter(sqlStringCommand, "Pid", DbType.String, string.Concat("B", dateTime2.ToString("yyMMddHHmmssfff")));
                this.database.AddInParameter(sqlStringCommand, "UserId", DbType.Int32, "0");
                this.database.AddInParameter(sqlStringCommand, "BuyTime", DbType.DateTime, dateTime2);
		...
		public bool CheckIsAll(string Aid)
		{
			...
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
			this.database.AddInParameter(sqlStringCommand, "ActivityId", System.Data.DbType.String, Aid);
			...
			if (list.Count < 1)
			{
				this.SetIsAllRefund(new List<string>
				{
					Aid
				});
				result = true;
