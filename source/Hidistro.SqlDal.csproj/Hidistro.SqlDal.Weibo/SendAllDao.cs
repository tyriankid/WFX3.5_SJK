...
namespace Hidistro.SqlDal.Weibo
{
	public class SendAllDao
	{
		...
		public System.Data.DataTable GetRencentOpenID(int topnum)
		{
			...
			string query = "select top " + num + " a.OpenID from WeiXin_RecentOpenID a inner join aspnet_Members b on a.openid=b.openid where b.IsFollowWeixin=1 and DATEADD(ss,172800, PubTime)>getdate()  order by PubTime desc";
			System.Data.Common.DbCommand sqlStringCommand = this.database.GetSqlStringCommand(query);
