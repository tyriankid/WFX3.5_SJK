		...
		protected override void AttachChildControls()
		{
			...
			foreach (LineItemInfo current in orderInfo.LineItems.Values)
			{
				if (current.SkuId.ToString() == this.SKuId)
				{
					...
					if (current.ItemAdjustedPrice == current.BalancePayMoney)
					{
						value = "1";
					}
					else if (current.BalancePayMoney == 0m)
					{
						value = "2";
					}
			...
			System.Web.UI.HtmlControls.HtmlInputHidden htmlInputHidden = (System.Web.UI.HtmlControls.HtmlInputHidden)this.FindControl("payways");
