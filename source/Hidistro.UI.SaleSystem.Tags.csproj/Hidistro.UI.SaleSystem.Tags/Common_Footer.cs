...
namespace Hidistro.UI.SaleSystem.Tags
{
	public class Common_Footer : SimpleTemplatedWebControl
	{
		...
		protected override void AttachChildControls()
		{
			...
			if (this.isShowNav == 1)
			{
				text = text + "<div style='padding-top:60px'></div><script>$(function () { jQuery.getJSON('/api/Hi_Ajax_NavMenu.ashx?" + DateTime.Now.ToString("MMddHHmm") + "', function (settingjson) {   $(_.template($('#menu').html())(settingjson)).appendTo('body'); GetUICss();});  }) </script>";
			}
