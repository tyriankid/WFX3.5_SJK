...
namespace Hidistro.UI.Web.API
{
	public class VshopProcess : System.Web.IHttpHandler, System.Web.SessionState.IRequiresSessionState
	{
        ...
        private void GetSiteRegions(HttpContext context)
        {
            ...
            {
                string s = "{\"success\":-1, \"msg\":\"请先扫描站点二维码后再来下单哦！\"}";
                context.Response.Write(s);
                context.Response.End();
                return;
            }
            DataTable dtSiteRegions = VShopHelper.GetScopeListBySiteId(currentMember.ReferralUserId);
            StringBuilder builder = new StringBuilder();
            builder.Append(string.Format("{{\"state\":{0},\"Count\":{1},\"Data\":", dtSiteRegions.Rows.Count > 0 ? 0 : 1, dtSiteRegions.Rows.Count));
            builder.Append(JsonConvert.SerializeObject(dtSiteRegions, Formatting.Indented).TrimStart('{').TrimEnd('}'));
            builder.Append("}");
        ...
        private void GetDistributorInfo(System.Web.HttpContext context)
		{
			...
			{
				distributorsInfo = DistributorsBrower.GetDistributorInfo(currentDistributorId);
				if (distributorsInfo != null)
				{
					distributorsInfo.Logo = Globals.GetWebUrlStart() + distributorsInfo.Logo;
				}
			}
			else
			{
				distributorsInfo.StoreName = masterSettings.SiteName;
				distributorsInfo.Logo = Globals.GetWebUrlStart() + masterSettings.DistributorLogoPic;
			}
			context.Response.ContentType = "application/json";
			context.Response.Write(JsonConvert.SerializeObject(distributorsInfo));
			context.Response.End();
		}

		private void ProcessUserPointByAdmin(System.Web.HttpContext context)
		{
			context.Response.ContentType = "application/json";
			int num = Globals.RequestFormNum("userid");
			ManagerInfo currentManager = ManagerHelper.GetCurrentManager();
			MemberHelper.GetMember(num);
		...
		private void GetSiteSettings(System.Web.HttpContext context)
		{
			SiteSettings masterSettings = SettingsManager.GetMasterSettings(true);
			ForceFollowInfo forceFollowInfo = new ForceFollowInfo();
			if (masterSettings != null)
			{
				forceFollowInfo.EnableGuidePageSet = masterSettings.EnableGuidePageSet;
				forceFollowInfo.IsAutoGuide = masterSettings.IsAutoGuide;
				forceFollowInfo.IsMustConcern = masterSettings.IsMustConcern;
				forceFollowInfo.GuideConcernType = masterSettings.GuideConcernType;
				forceFollowInfo.GuidePageSet = masterSettings.GuidePageSet;
				forceFollowInfo.ConcernMsg = masterSettings.ConcernMsg;
				if (masterSettings.IsMustConcern)
				{
					forceFollowInfo.IsAutoGuide = true;
				}
			}
			int followInfo = this.CheckFoucs();
			forceFollowInfo.FollowInfo = followInfo;
			context.Response.ContentType = "application/json";
			context.Response.Write(JsonConvert.SerializeObject(forceFollowInfo));
			context.Response.End();
		}

		private void OnekeySet(System.Web.HttpContext context)
		{
			WxJsonResult value = WxTemplateSendHelp.QuickSetWeixinTemplates();
			context.Response.ContentType = "application/json";
		...
		public string MemberAutoToDistributor(MemberInfo memberinfo)
		{
			...
			text = DistributorsBrower.MemberAutoToDistributor(memberinfo);
			if (text == "1")
			{
				...
				if (httpCookie != null)
				{
					...
					httpCookie2.Value = memberinfo.UserId.ToString();
			...
			{
				Globals.Debuglog("自动生成分销商失败，原因是：" + text, "_DebuglogMemberAutoToDistributor.txt");
			}
			return text;
		}
		...
		private void GetSecondDistributors(System.Web.HttpContext context)
		{
			...
			int num2 = 0;
			string text = context.Request["sort"];
			if (string.IsNullOrWhiteSpace(text))
			{
				text = "CreateTime";
			}
			string text2 = context.Request["order"];
			if (string.IsNullOrWhiteSpace(text2))
			{
				text2 = "desc";
			}
			System.Data.DataTable downDistributors = DistributorsBrower.GetDownDistributors(distributorsQuery, out num2, text, text2);
		...
		public string GetSecondDistributorsHtml(System.Data.DataTable dt)
		{
			...
			if (dt.Rows.Count > 0)
			{
				for (int i = 0; i < dt.Rows.Count; i++)
				{
					...
					{
						" <h3> ",
						Globals.String2Json(dt.Rows[i]["StoreName"].ToString()),
						"【",
						Globals.String2Json(dt.Rows[i]["GradeName"].ToString()),
						"】</h3>"
					}));
					...
					stringBuilder.Append("<p><span class='colorc'>用户呢称：</span>" + Globals.String2Json(dt.Rows[i]["UserName"].ToString()) + "</p>");
					...
					stringBuilder.Append(string.Concat(new string[]
					{
						...
						string.IsNullOrEmpty(dt.Rows[i]["OrderTotal"].ToString()) ? "0.00" : string.Format("{0:F2}", dt.Rows[i]["OrderTotal"]),
						"</span></a></p>"
					}));
					stringBuilder.Append("</div>");
					...
					stringBuilder.Append(string.Concat(new string[]
					{
						...
						string.IsNullOrEmpty(dt.Rows[i]["CommTotal"].ToString()) ? "0.00" : string.Format("{0:F2}", dt.Rows[i]["CommTotal"]),
						"</span></a></p>"
					}));
					stringBuilder.Append("</div>");
					stringBuilder.Append("</div>");
		...
		private void GetMyMember(System.Web.HttpContext context)
		{
			...
			System.Data.DataTable membersByUserId = MemberProcessor.GetMembersByUserId(referralUserId, pageIndex, pageSize, out num, text, text2);
		...
		private string GetMyMemberHtml(System.Data.DataTable dt)
		{
			...
			if (dt.Rows.Count > 0)
			{
				for (int i = 0; i < dt.Rows.Count; i++)
				{
					...
					stringBuilder.Append("<p><span class='colorc'>消费金额：</span><span class='colorg'>￥" + (string.IsNullOrEmpty(dt.Rows[i]["OrdersTotal"].ToString()) ? "0" : string.Format("{0:F2}", dt.Rows[i]["OrdersTotal"])) + "</span></p>");
					stringBuilder.Append("</div>");
					stringBuilder.Append("</div>");
					stringBuilder.Append("</div>");
					stringBuilder.Append("<span class='left radius'></span>");
		...
		public int CheckFoucs()
		{
            ...
            if (currentMemberUserId != 0)
            {
                ...
                if (currentMember != null)
                {
                    ...
                    {
                        SiteSettings masterSettings = SettingsManager.GetMasterSettings(true);
                        string tokenMessage = TokenApi.GetToken_Message(masterSettings.WeixinAppId, masterSettings.WeixinAppSecret);
                        if (tokenMessage.Contains("errmsg") && tokenMessage.Contains("errcode"))
                        {
                            isFollowWeixin = 2;
                        }
                        else if (!string.IsNullOrEmpty(currentMember.OpenId))
                        {
                            if (JsonConvert.DeserializeAnonymousType(BarCodeApi.GetUserInfosByOpenID(tokenMessage, currentMember.OpenId), new { subscribe = "", nickname = "", headimgurl = "" }).subscribe.Trim() == "1")
                            {
                                isFollowWeixin = 1;
		...
		public void followCheck(System.Web.HttpContext context)
		{
            ...
            empty = "{\"type\":\"0\",\"tips\":\"未知错误！\"}";
            int currentMemberUserId = Globals.GetCurrentMemberUserId(false);
            string item = context.Request["followtype"];
            if (currentMemberUserId == 0)
            {
                empty = "{\"type\":\"1\",\"tips\":\"当前用户未登入！\"}";
            }
            else if (!string.IsNullOrEmpty(item))
            {
                ...
                if (currentMember != null)
                {
                    if (item == "wx" && !string.IsNullOrEmpty(currentMember.OpenId))
                    {
                        isFollowWeixin = currentMember.IsFollowWeixin;
                        if (isFollowWeixin == 0)
                        {
                            ...
                            else if (JsonConvert.DeserializeAnonymousType(BarCodeApi.GetUserInfosByOpenID(tokenMessage, currentMember.OpenId), new { subscribe = "", nickname = "", headimgurl = "" }).subscribe.Trim() == "1")
                            {
                                isFollowWeixin = 1;
                                MemberProcessor.UpdateUserFollowStateByUserId(currentMemberUserId, 1, "wx");
                            }
                    ...
                    else if (!(item == "fw") || string.IsNullOrEmpty(currentMember.AlipayOpenid))
                    {
                        isFollowWeixin = 2;
                    }
                    else
                    ...
                    if (isFollowWeixin != 0)
                    {
                        empty = (isFollowWeixin != 1 ? "{\"type\":\"4\",\"tips\":\"非正常访问！！\"}" : "{\"type\":\"3\",\"tips\":\"当前用已关注！\"}");
                    }
                    else
                    {
                        empty = "{\"type\":\"2\",\"tips\":\"当前用户未关注！\"}";
                    }
                }
                else
                {
                    empty = "{\"type\":\"0\",\"tips\":\"用户不存在！\"}";
                }
            }
            else
            {
                empty = "{\"type\":\"0\",\"tips\":\"非法访问！\"}";
            }
		...
		public void BindOldUserName(System.Web.HttpContext context)
		{
			...
			if (!string.IsNullOrEmpty(text))
			{
				...
				if (memberInfo.OpenId != null && memberInfo.OpenId.Length > 0)
				{
					stringBuilder.Append("\"Status\":\"-1\",\"Msg\":\"帐号已经绑定过其他微信！\"");
					stringBuilder.Append("}");
					context.Response.Write(stringBuilder.ToString());
					return;
				}
		...
		private void AddProductConsultations(System.Web.HttpContext context)
		{
			...
			if (ProductBrowser.InsertProductConsultation(productConsultationInfo))
			{
				...
				try
				{
					...
					{
						Messenger.SendWeiXinMsg_ProductAsk(productBaseInfo.ProductName, "", productConsultationInfo.ConsultationText, productConsultationInfo.UserName);
					}
					return;
				}
				catch (System.Exception)
			...
			if (orderInfo != null && MemberProcessor.ConfirmOrderFinish(orderInfo))
			{
				...
				if (masterSettings.IsRequestDistributor)
				{
					...
					if (num != 1 && !string.IsNullOrEmpty(masterSettings.DistributorProducts) && masterSettings.DistributorProducts.ToString() != "0")
					{
						...
						{
							array.ToList<string>();
							foreach (System.Collections.Generic.KeyValuePair<string, LineItemInfo> current2 in lineItems)
							{
								LineItemInfo value = current2.Value;
								if (array.Contains(value.ProductId.ToString()))
								{
									num = 1;
									break;
								}
				...
				{
					if (current3.OrderItemsStatus.ToString() == OrderStatus.SellerAlreadySent.ToString())
					{
						ShoppingProcessor.UpdateOrderGoodStatu(orderInfo.OrderId, current3.SkuId, 5, current3.ID);
					}
				}
				DistributorsInfo distributorsInfo = new DistributorsInfo();
		...
		{
			string s = string.Empty;
			System.Uri urlReferrer = context.Request.UrlReferrer;
				...
				if (int.TryParse(text2, out num) && num > 0 && oneyuanTaoInfoById != null)
				{
					...
					else
					{
						...
						else
						{
							...
							int userAlreadyBuyNum = OneyuanTaoHelp.GetUserAlreadyBuyNum(currentMember.UserId, text);
							if (num + userAlreadyBuyNum > eachCanBuyNum)
							{
								stringBuilder.Append("\"Status\":false,\"Msg\":\"已经超出最大购买份数！！\"");
							}
							else
							{
								oneyuanTaoParticipantInfo.Pid = OneyuanTaoHelp.GetOrderNumber(false);
								oneyuanTaoParticipantInfo.ActivityId = text;
								oneyuanTaoParticipantInfo.UserId = Globals.GetCurrentMemberUserId(false);
		...
		public string ordersummit(ShoppingCartInfo cart, System.Web.HttpContext context, string remark, int shippingId, string couponCode, string selectCouponValue, string shippingTypeinfo, bool summittype, string OrderMarking, System.Collections.Generic.IList<ShoppingCartItemInfo> ItemInfo, int PointExchange, int bargainDetialId, out string ActivitiesIds)
		{
			...
			if (orderInfo != null)
			{
				...
				{
					orderInfo.DiscountAmount = this.DiscountMoney(ItemInfo, out text, out text2, currentMember, out num, out empty);
				}
				ActivitiesIds = text;
				...
				orderInfo.ActivitiesId = text.Trim(new char[]
				{
					','
				});
				orderInfo.ActivitiesName = text2.Trim(new char[]
				{
					','
				});
				...
				int num2 = Globals.GetCurrentDistributorId();
				if (num2 > 0)
				{
					DistributorsInfo distributorInfo = DistributorsBrower.GetDistributorInfo(num2);
					if (distributorInfo != null)
					{
						...
						case 0:
							...
							break;
						case 1:
							if (num2 == orderInfo.UserId)
							{
								MemberInfo member = MemberHelper.GetMember(num2);
								if (member != null)
								{
									num2 = member.ReferralUserId;
									DistributorsInfo distributorInfo2 = DistributorsBrower.GetDistributorInfo(num2);
				...
				if (int.TryParse(shippingTypeinfo, out num3))
				{
					orderInfo.ShippingModeId = num3;
					orderInfo.ModeName = this.getModelType(num3);
					...
					if (num3 > 0)
					{
						...
						{
							System.Collections.Generic.Dictionary<int, ShoppingCartItemInfo> dictionary = new System.Collections.Generic.Dictionary<int, ShoppingCartItemInfo>();
							foreach (ShoppingCartItemInfo current in cart.LineItems)
							{
								if (!dictionary.ContainsKey(current.FreightTemplateId))
								{
									current.SumSubTotal = current.SubTotal;
							...
							foreach (System.Collections.Generic.KeyValuePair<int, ShoppingCartItemInfo> current2 in dictionary)
							{
								cart.LineItems.Add(current2.Value);
							}
							foreach (ShoppingCartItemInfo current3 in cart.LineItems)
							{
								if (!current3.IsfreeShipping)
								{
									bool flag2 = false;
									...
									if (templateMessage != null && current3.FreightTemplateId > 0 && !templateMessage.FreeShip)
									{
										if (templateMessage.HasFree)
										{
											flag2 = this.IsFreeTemplateShipping(context.Request["Shippingcity"], current3.FreightTemplateId, num3, current3);
										}
										if (!flag2)
										{
											dataView.RowFilter = string.Concat(new object[]
													...
													if (!(a == "1"))
													{
														if (!(a == "2"))
														{
															if (a == "3")
															{
																if (current3.CubicMeter > 0m)
																{
																	num5 += this.setferight(float.Parse(current3.CubicMeter.ToString()), float.Parse(dataView[0]["FristNumber"].ToString()), float.Parse(dataView[0]["FristPrice"].ToString()), float.Parse(dataView[0]["AddNumber"].ToString()), float.Parse(dataView[0]["AddPrice"].ToString()));
																}
															}
														}
														else if (current3.FreightWeight > 0m)
														{
															num5 += this.setferight(float.Parse(current3.FreightWeight.ToString()), float.Parse(dataView[0]["FristNumber"].ToString()), float.Parse(dataView[0]["FristPrice"].ToString()), float.Parse(dataView[0]["AddNumber"].ToString()), float.Parse(dataView[0]["AddPrice"].ToString()));
														}
													}
													else
													{
														num5 += this.setferight(float.Parse(current3.Quantity.ToString()), float.Parse(dataView[0]["FristNumber"].ToString()), float.Parse(dataView[0]["FristPrice"].ToString()), float.Parse(dataView[0]["AddNumber"].ToString()), float.Parse(dataView[0]["AddPrice"].ToString()));
													}
												}
											}
											else
						...
						string s = num5.ToString("F2");
						orderInfo.AdjustedFreight = decimal.Parse(s);
					}
				}
				if (int.TryParse(context.Request["paymentType"], out num4))
				{
					orderInfo.PaymentTypeId = num4;
					if (num4 == 0 || num4 == -1)
					{
					...
					{
						PaymentModeInfo paymentMode = ShoppingProcessor.GetPaymentMode(num4);
						if (paymentMode != null)
						{
                ...
                //如果奶卡是自己的,才可享受免费
                if (orderInfo.MilkCardId != null && orderInfo.UserId == cart.MilkCard.UserId)
                {
                    amount = 0m;
                }
				...
				{
					stringBuilder.Append("\"Status\":\"Error\"");
					stringBuilder.AppendFormat(",\"ErrorMsg\":\"优惠券金额不正确，请重新提交订单！\"", new object[0]);
					return stringBuilder.ToString();
				}
				orderInfo.CouponFreightMoneyTotal = 0m;
				decimal num6;
				if (amount + orderInfo.AdjustedFreight <= orderInfo.RedPagerAmount)
				{
					orderInfo.RedPagerAmount = amount + orderInfo.AdjustedFreight;
					orderInfo.CouponFreightMoneyTotal = orderInfo.AdjustedFreight;
					num6 = 0m;
				}
				else if (amount <= orderInfo.RedPagerAmount)
				{
					orderInfo.CouponFreightMoneyTotal = orderInfo.RedPagerAmount - amount;
					num6 = 0m;
				}
				else
				{
					num6 = amount - orderInfo.RedPagerAmount;
				}
				if (masterSettings.PonitToCash_Enable && bargainDetialId == 0 && flag)
				{
					...
					int points = currentMember.Points;
					int num7 = points - orderInfo.GetTotalPointNumber();
					int num8 = (int)(ponitToCash_MaxAmount * pointToCashRate);
					if (num7 > num8)
					{
						num7 = num8;
					}
					...
					int num10 = (int)(pointToCashRate * num9);
					if (num10 > num7)
					{
						num10 = num7;
					}
					orderInfo.PointExchange = num10;
                    decimal ss = num10 / pointToCashRate;
					orderInfo.PointToCash = System.Math.Round(ss, 2);
				}
				try
				{
					decimal remainingMondy = 0m;
					this.SetOrderItemStatus(orderInfo, orderInfo.RedPagerAmount - orderInfo.CouponFreightMoneyTotal, orderInfo.PointToCash, orderInfo.DiscountAmount, empty, out remainingMondy);
					...
					if (bargainInfoByDetialId == null || (bargainInfoByDetialId != null && bargainInfoByDetialId.IsCommission))
					{
						if (orderInfo.BargainDetialId > 0)
						{
							...
							if (bargainDetialInfo != null)
							{
								...
								foreach (LineItemInfo current4 in values)
								{
									current4.ItemAdjustedPrice = bargainDetialInfo.Price;
									current4.ItemListPrice = bargainDetialInfo.Price;
									current4.CommissionDiscount = bargainInfoByDetialId.CommissionDiscount;
								}
							}
						}
					...
					orderInfo.logisticsTools = LogisticsTools.Kuaidi100;
					if (!string.IsNullOrEmpty(masterSettings.Exp_appKey) && !string.IsNullOrEmpty(masterSettings.Exp_appSecret))
					{
						orderInfo.logisticsTools = LogisticsTools.Kuaidiniao;
					}
                    //写入订单实体类到数据库
					int num11 = ShoppingProcessor.CreatOrder(orderInfo, isUseBalance, remainingMondy);
					if (num11 > 0)
					{
						...
						if (num11 == 2)
						{
							...
							MemberHelper.SetOrderDate(orderInfo.UserId, 1);
						}
						if (summittype)
						{
		...
		private void ProcessSubmmitorder(System.Web.HttpContext context)
		{
			...
			int num2;
			int.TryParse(context.Request["groupbuyId"], out num2);
			int num3 = 0;
			string remark = context.Request["remark"];
            ...
            if (int.TryParse(context.Request["buyAmount"], out num4) && !string.IsNullOrEmpty(context.Request["productSku"]) && !string.IsNullOrEmpty(context.Request["from"]) && (context.Request["from"] == "signBuy" || context.Request["from"] == "groupBuy"))
			{
				...
				if (num > 0)
				{
					bool flag2 = BargainHelper.UpdateNumberById(num, num4, out num3);
					if (flag2)
					{
						list = ShoppingCartProcessor.GetListShoppingCart(text5, num3, num, 0);
					}
				}
				else
				{
					int num5 = Globals.RequestFormNum("limitedTimeDiscountId");
					int num6 = num4;
					if (num5 > 0)
					{
						bool flag3 = true;
						LimitedTimeDiscountInfo discountInfo = LimitedTimeDiscountHelper.GetDiscountInfo(num5);
						if (discountInfo == null)
						{
							flag3 = false;
						}
						if (flag3)
						{
							int limitedTimeDiscountUsedNum = ShoppingCartProcessor.GetLimitedTimeDiscountUsedNum(num5, text5, 0, currentMember.UserId, false);
							if (discountInfo.LimitNumber > 0 && num4 > discountInfo.LimitNumber - limitedTimeDiscountUsedNum)
							{
								...
								{
									num6 = discountInfo.LimitNumber - limitedTimeDiscountUsedNum;
								}
								else
								{
									num6 = 0;
                ...
                using (System.Collections.Generic.List<ShoppingCartInfo>.Enumerator enumerator = list.GetEnumerator())
				{
					while (enumerator.MoveNext())
					{
						...
						string value = this.ordersummit(current, context, remark, shippingId, couponCode, array[0], context.Request["shippingType"], false, text4, current.LineItems, 0, num, out text3);
						stringBuilder.Append(value);
						text2 = text3 + ",";
					}
					goto IL_550;
			...
			string[] array2 = context.Request["shippingType"].Split(new char[]
			{
				','
			});
			string[] array3 = context.Request["remark"].Split(new char[]
			{
				','
			});
			int num7 = 0;
			...
			foreach (ShoppingCartInfo current2 in list2)
			{
				foreach (ShoppingCartItemInfo current3 in current2.LineItems)
				{
					if (current3.Type == 1)
					{
						num8 += current3.PointNumber;
					}
				}
			}


            if (num8 > currentMember.Points)
			{
			...
			else
			{
				...
				{
                    //创建订单
					this.ordersummit(current4, context, array3[num7], shippingId, couponCode, array[num7], array2[num7], true, text4, current4.LineItems, 0, 0, out text3);
					text2 = text2 + text3 + ",";
					num7++;
				}
								...
								if (bool.Parse(activities.Rows[i]["isAllProduct"].ToString()))
								{
									if (decimal.Parse(activities_Detail.Rows[j]["MeetMoney"].ToString()) > 0m)
									{
										if (d2 != 0m && d2 >= decimal.Parse(activities_Detail.Rows[activities_Detail.Rows.Count - 1]["MeetMoney"].ToString()))
										{
											...
											ActivitiesId = ActivitiesId + activities_Detail.Rows[activities_Detail.Rows.Count - 1]["id"].ToString() + ",";
											ActivitiesName = ActivitiesName + activities_Detail.Rows[activities_Detail.Rows.Count - 1]["ActivitiesName"].ToString() + ",";
										...
										if (d2 != 0m && d2 >= decimal.Parse(activities_Detail.Rows[j]["MeetMoney"].ToString()))
										{
											...
											ActivitiesId = ActivitiesId + activities_Detail.Rows[j]["id"].ToString() + ",";
											ActivitiesName = ActivitiesName + activities_Detail.Rows[j]["ActivitiesName"].ToString() + ",";
		...
		public void SetOrderItemStatus(OrderInfo order, decimal redPagerAmount, decimal pointDiscountAverage, decimal DiscountAverage, string productItemList, out decimal remainingMondy)
		{
				...
				using (System.Collections.Generic.Dictionary<string, LineItemInfo>.Enumerator enumerator2 = lineItems.GetEnumerator())
				{
					while (enumerator2.MoveNext())
					{
						...
						if (lineItemInfo.Type == 0)
						{
							...
							if (lineItemInfo.DiscountAverage > lineItemInfo.ItemAdjustedPrice * lineItemInfo.Quantity)
							{
								remainingMondy += lineItemInfo.DiscountAverage - lineItemInfo.ItemAdjustedPrice * lineItemInfo.Quantity;
								lineItemInfo.DiscountAverage = lineItemInfo.ItemAdjustedPrice * lineItemInfo.Quantity;
							}
						}
						else
						{
			...
			if (lineItems.Count == 1)
			{
				foreach (System.Collections.Generic.KeyValuePair<string, LineItemInfo> arg_3D7_0 in lineItems)
				{
					if (lineItemInfo.Type == 0)
					{
		...
		public void AddDistributor(System.Web.HttpContext context)
		{
			...
			else
			{
				...
				if (!flag && masterSettings.RechargeMoneyToDistributor > 0m && MemberAmountProcessor.GetUserMaxAmountDetailed(currentMember.UserId) >= masterSettings.RechargeMoneyToDistributor)
				{
					flag = true;
				}
			}
            ...
            distributorsInfo.SiteTel = context.Request["CellPhone"].Trim(); 
            distributorsInfo.SiteRegionId = Convert.ToInt32(context.Request["regionSelectorValue"].Trim());
            distributorsInfo.SiteAddress = context.Request["address"].Trim();
            distributorsInfo.SiteStatus = 0; //默认申请成功后为正常开店状态
			...
			if (DistributorsBrower.AddDistributors(distributorsInfo))
			{
				...
				if (httpCookie != null)
				{
					...
					System.Web.HttpContext.Current.Response.Cookies[name].Expires = System.DateTime.Now.AddDays(-1.0);
					System.Web.HttpCookie httpCookie2 = new System.Web.HttpCookie(name);
					httpCookie2.Value = Globals.GetCurrentMemberUserId(false).ToString();
					httpCookie2.Expires = System.DateTime.Now.AddYears(10);
					System.Web.HttpContext.Current.Response.Cookies.Add(httpCookie2);
				}
				this.myNotifier.updateAction = UpdateAction.MemberUpdate;
		...
		private void AddCommissions(System.Web.HttpContext context)
		{
			...
			if (!DistributorsBrower.IsExitsCommionsRequest())
			{
				if (this.CheckAddCommissions(context, ref s))
				{
					...
					balanceDrawRequestInfo.RequestType = num;
					MemberInfo currentMember = MemberProcessor.GetCurrentMember();
					if (text == "")
					{
