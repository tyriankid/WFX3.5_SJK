...
namespace Hidistro.UI.Web.Admin.Bargain
{
	public class AddBargain : AdminPage
	{
		...
		private void btnSave_Click(object sender, System.EventArgs e)
		{
			...
			bool @checked = this.ckIsCommission.Checked;
			string text5 = this.discount.Text;
			if (@checked && string.IsNullOrEmpty(text5))
			{
			...
			bargainInfo.CommissionDiscount = (@checked ? ((int)(float.Parse(text5) * 10f)) : 0);
			if (num7 == 0)
			{
				string text6 = this.txtBargainTypeOneValue.Text;
				if (string.IsNullOrWhiteSpace(text6))
				{
					this.ShowMsg("每次砍掉价格不能为空！", false);
					return;
				}
				bargainInfo.BargainTypeMinVlue = float.Parse(text6);
			}
			else
			{
				string text7 = this.txtBargainTypeTwoValue1.Text;
				string text8 = this.txtBargainTypeTwoValue2.Text;
				if (string.IsNullOrWhiteSpace(text7) || string.IsNullOrWhiteSpace(text8))
				{
				...
				float num8 = 0f;
				float num9 = 0f;
				if (!float.TryParse(text7, out num8))
				{
					this.ShowMsg("随机砍价最小值必须为数值！", false);
					return;
				}
				if (!float.TryParse(text8, out num9))
				{
