...
namespace Hidistro.UI.Web.Admin.Oneyuan
{
	public class RefoundList : AdminPage
	{
		...
		private void AjaxAction(string action)
		{
            ...
            if (str1 != null)
            {
                if (str2 == "WeiXinRefund")
                {
                    string item = form["Pid"];
                    ...
                    {
                        OneyuanTaoParticipantInfo addParticipant = OneyuanTaoHelp.GetAddParticipant(0, item, "");
                        if (addParticipant == null || !addParticipant.IsPay)
                        {
                            str = "{\"state\":false,\"msg\":\"用户记录不存在或者用户未支付！\"}";
                        }
                        else if (addParticipant.IsRefund)
                        {
                            str = "{\"state\":false,\"msg\":\"该订单已退款！\"}";
                        }
                        else
                        {
                            string outRefundNo = addParticipant.out_refund_no;
                            if (string.IsNullOrEmpty(outRefundNo))
                            {
                                outRefundNo = RefundHelper.GenerateRefundOrderId();
                                OneyuanTaoHelp.Setout_refund_no(item, outRefundNo);
                            }
                            string str3 = "";
                            string str4 = RefundHelper.SendWxRefundRequest(item, addParticipant.TotalPrice, addParticipant.TotalPrice, outRefundNo, out str3);
                            if (str4 != "")
                            {
                                addParticipant.Remark = string.Concat("退款失败:", str4.Replace("OK", ""));
                                OneyuanTaoHelp.SetRefundinfoErr(addParticipant);
                                str = string.Concat("{\"state\":false,\"msg\":\"", addParticipant.Remark, "\"}");
                            }
                            else
                            {
                                str = "{\"state\":true,\"msg\":\"退款成功\"}";
                                addParticipant.Remark = "退款成功";
                                addParticipant.RefundNum = str3;
                ...
                else if (str2 == "WeiXinBacthRefund")
                {
                    string item1 = base.Request.Form["Pids"];
                    str = this.BatchWxRefund(item1);
                }
                else if (str2 == "BatchRefund")
                {
                    str = "{\"state\":false,\"msg\":\"初始化中！\"}";
                    string item2 = base.Request.Form["vaid"];
                    if (!string.IsNullOrEmpty(item2))
                    {
                        List<string> participantPids = OneyuanTaoHelp.GetParticipantPids(item2, true, false, "weixin");
                        if (participantPids.Count > 0)
                        {
                            string str5 = string.Join(",", participantPids);
                            (new Thread(() => Globals.Debuglog(this.BatchWxRefund(str5), "_wxrefund.txt"))).Start();
                        }
                        if (OneyuanTaoHelp.GetParticipantPids(item2, true, false, "alipay").Count > 0)
                        {
                            str = "{\"state\":true,\"msg\":\"转向支付宝退款中...！\",\"alipay\":true}";
                        }
                        else if (participantPids.Count <= 0)
                        {
                            str = "{\"state\":false,\"msg\":\"没有退款数据可以操作！\",\"alipay\":false}";
                            OneyuanTaoHelp.SetIsAllRefund(new List<string>()
                            {
                                item2
                            });
                        ...
                        {
                            str = "{\"state\":true,\"msg\":\"正在后台处理微信退款，请稍后刷新！\",\"alipay\":false}";
                        }
                    }
                    else
                    {
