			...
			if (System.IO.File.Exists(text))
			{
				...
				for (int i = 0; i < dataTable.Rows.Count; i++)
				{
					...
					foreach (XmlNode xmlNode2 in xmlNode.SelectNodes("item"))
					{
						...
						case "订单-详情":
						{
							...
							if (array3.Length > 0)
							{
								...
								for (int l = 0; l < array4.Length; l++)
								{
									...
									text10 = dataRow2["SKU"].ToString();
									if (string.IsNullOrEmpty(text10))
									{
										ProductInfo productDetails = ProductHelper.GetProductDetails(Globals.ToNum(dataRow2["ProductId"]));
										if (productDetails != null)
										{
											text10 = productDetails.ProductCode;
										}
									}
									text9 = string.Concat(new object[]
									{
										text9,
										(dataRow2["SKUContent"] != System.DBNull.Value && dataRow2["SKUContent"].ToString().Trim() != "") ? (this.GetFormatTitle(dataRow2["SKUContent"]) + " ") : "",
										"商品编码:",
										this.GetFormatTitle(text10),
										" 数量:",
							...
							if (num > i)
							{
								for (int m = i + 1; m < num + 1; m++)
								{
									...
									for (int n = 0; n < array5.Length; n++)
									{
										...
										text10 = dataRow3["SKU"].ToString();
										if (string.IsNullOrEmpty(text10))
										{
											ProductInfo productDetails2 = ProductHelper.GetProductDetails(Globals.ToNum(dataRow3["ProductId"]));
											if (productDetails2 != null)
											{
												text10 = productDetails2.ProductCode;
											}
										}
										text9 = string.Concat(new object[]
										{
											text9,
											(dataRow3["SKUContent"] != System.DBNull.Value && dataRow3["SKUContent"].ToString().Trim() != "") ? (this.GetFormatTitle(dataRow3["SKUContent"]) + " ") : "",
											"商品编码:",
											this.GetFormatTitle(text10),
											" 数量:",
						...
						{
							string[] array6 = innerText2.Split(new char[]
							{
								'_'
							});
							if (array6.Length == 3 && array6[1] == "自定义内容")
							{
								this.SelfDefinedContentLength++;
								this.SelfDefinedContent = this.SelfDefinedContent + "'" + this.ReplaceString(array6[0]) + "',";
