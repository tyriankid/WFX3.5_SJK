		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if ((a = text) != null)
			{
				if (a == "saveorders")
				{
					...
					if (jArray != null)
					{
						...
						if (flag)
						{
							...
							s = "{\"type\":\"1\",\"tips\":\"" + str + "指定物流成功！\"}";
						}
						else
						{
							s = "{\"type\":\"0\",\"tips\":\"" + str + "指定物流失败，请检测数据的正确性！\"}";
						}
					}
				...
				if (a == "sendorders")
				{
					...
					if (jArray != null)
					{
						...
						if (flag3)
						{
							...
							if (num == 0)
							{
								s = "{\"type\":\"0\",\"tips\":\"" + str + "发货失败！\"}";
							}
							else
							{
								s = "{\"type\":\"1\",\"tips\":\"" + num + "个订单发货成功！\"}";
							}
						}
						else
						{
							s = "{\"type\":\"0\",\"tips\":\"" + str + "发货失败，请检测数据的正确性！\"}";
						}
					}
					base.Response.Write(s);
					base.Response.End();
				...
				if (a == "updateExpress")
				{
					...
					JArray jArray = (JArray)JsonConvert.DeserializeObject(text2);
					bool flag4 = true;
					using (System.Collections.Generic.IEnumerator<JToken> enumerator8 = jArray.GetEnumerator())
					{
						while (enumerator8.MoveNext())
						{
							JObject jObject7 = (JObject)enumerator8.Current;
							if (!this.CheckOrderCompany(jObject7["orderid"].ToString(), jObject7["companycode"].ToString(), jObject7["compname"].ToString(), jObject7["shipordernumber"].ToString()))
							{
								flag4 = false;
							}
						}
					...
					{
						bool flag5 = false;
						using (System.Collections.Generic.IEnumerator<JToken> enumerator9 = jArray.GetEnumerator())
						{
							while (enumerator9.MoveNext())
							{
								JObject jObject8 = (JObject)enumerator9.Current;
								OrderInfo orderInfo2 = OrderHelper.GetOrderInfo(jObject8["orderid"].ToString());
								orderInfo2.ExpressCompanyAbb = jObject8["companycode"].ToString();
								orderInfo2.ExpressCompanyName = jObject8["compname"].ToString();
								orderInfo2.ShipOrderNumber = jObject8["shipordernumber"].ToString();
								flag5 = OrderHelper.UpdateOrder(orderInfo2);
								if (flag5)
								{
									Express.SubscribeExpress100(jObject8["companycode"].ToString(), jObject8["shipordernumber"].ToString());
								}
							}
						}
						if (flag5)
