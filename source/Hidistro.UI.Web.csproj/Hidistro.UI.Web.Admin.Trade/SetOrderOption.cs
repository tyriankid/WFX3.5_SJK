...
namespace Hidistro.UI.Web.Admin.Trade
{
	public class SetOrderOption : AdminPage
	{
		...
		protected void btnSave_Click(object sender, System.EventArgs e)
		{
			...
			int maxReturnedDays;
			decimal taxRate;
			if (!this.ValidateValues(out orderShowDays, out closeOrderDays, out maxReturnedDays, out taxRate))
			{
				return;
			}
			SiteSettings masterSettings = SettingsManager.GetMasterSettings(false);
		...
		private bool ValidateValues(out int showDays, out int closeOrderDays,  out int maxReturnedDays, out decimal taxRate)
		{
