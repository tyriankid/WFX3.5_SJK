namespace Hishop.WeiBo.Api
{
    ...
    public abstract class OpenAuthenticationClientBase
    {
        ...
        public virtual Task<HttpResponseMessage> HttpGetAsync(string api, Dictionary<string, object> parameters = null)
        {
            ...
            string str = string.Join("&", (IEnumerable<string>)(from p in parameters
                                                                where ((p.Value == null) || p.Value.GetType().IsValueType) || (p.Value.GetType() == typeof(string))
        ...
        public Task<HttpResponseMessage> HttpGetAsync(string api, object parameters)
        {
            ...
            Dictionary<string, object> dictionary = (from p in type.GetProperties()
                                                     where p.PropertyType.IsValueType || (p.PropertyType == typeof(string))
                                                     select p).ToDictionary<PropertyInfo, string, object>(k => k.Name, v => string.Format("{0}", v.GetValue((dynamic)parameters, (dynamic)null)));
        ...
        public virtual Task<HttpResponseMessage> HttpPostAsync(string api, Dictionary<string, object> parameters)
        {
            ...
            if (source.Count<KeyValuePair<string, object>>(p => ((p.Value.GetType() == typeof(byte[])) || (p.Value.GetType() == typeof(FileInfo)))) > 0)
            {
