	...
	public class HttpService
	{
		...
		public static string Post(string xml, string url, bool isUseCert, PayConfig config, int timeout)
		{
			...
			try
			{
				...
				{
					byte[] array;
					using (FileStream fileStream = new FileStream(config.SSLCERT_PATH, FileMode.Open, FileAccess.Read))
					{
						array = new byte[fileStream.Length];
						fileStream.Read(array, 0, array.Length);
						fileStream.Close();
					}
					X509Certificate2 value = new X509Certificate2(array, config.SSLCERT_PASSWORD, X509KeyStorageFlags.MachineKeySet | X509KeyStorageFlags.PersistKeySet);
