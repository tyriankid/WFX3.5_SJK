                ...
                <a href="<%# "/ProductDetails.aspx?productId=" + Eval("ProductId")%>" class="detailLink">
                    <div class="box">
                        ...
                        <div class="right">
                            ...
                            <div class="price text-danger" style="display:none">
                                <%# Eval("Type").ToString()=="0"?"":"积分兑换商品"%>
                                <%# Eval("Type").ToString()=="0"?"￥"+Eval("AdjustedPrice", "{0:F2}"):Eval("PointNumber")+"积分"%>
                                <span class="bcolor" role="quantityPerDay" value="<%# Eval("QuantityPerDay")%>"> x <%# Eval("QuantityPerDay")%>(瓶/天) </span>
                                <span class="bcolor" role="sendDays" value="<%# Eval("SendDays")%>">x <%# Eval("SendDays")%>天</span>  
                            </div>
                        </div>
                        <div class="list1">
                            <ul>
                                
                                <li role="sendStartDate" value="<%# ((DateTime)Eval("SendStartDate")).ToString("yyyy-MM-dd")%>">起订日期：<%# ((DateTime)Eval("SendStartDate")).ToString("yyyy-MM-dd")%></li>
                                <li>截止日期：<%# ((DateTime)Eval("SendEndDate")).ToString("yyyy-MM-dd")%></li>
                                <li>单价：<%# Eval("Type").ToString()=="0"?"￥"+Eval("AdjustedPrice", "{0:F2}"):Eval("PointNumber")+"积分"%></li>
                                <li>总天数：<%# Eval("SendDays")%>天</li>
                                <li>总数量：<%# Convert.ToInt32(Eval("QuantityPerDay"))*Convert.ToInt32(Eval("SendDays"))%>瓶</li>
                            </ul>
