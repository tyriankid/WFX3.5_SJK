$(document).ready(function () {
    ...
    $("#btnOrderMilk").bind("click", function () { OrderMilk(); });
...
function BuyProduct() {
    ...
    if (limitNumber > 0 && limitedTimeDiscountId > 0 && quantity > remainlimitedTimeDiscountNum) {
        ...
        location.href = "/Vshop/Orderbycycle.aspx?productid=" + getQueryString("productId") + "&productSku=" + $("#hiddenSkuId").val() + "&ReferralId=" + getParam("ReferralId");
        return false;
...
function OrderMilk() {
    var quantityPerDay = $("#buyNum").val();
    var sendDays = $("[role='strSendDays']").html();
    var startDate = $("#startDate").val();

    location.href = "/Vshop/SubmmitOrder.aspx?buyAmount=" + 0 + "&productSku=" + $("#hidProductSku").val()/*getQueryString("productSku")*/ + "&limitedTimeDiscountId=" + 0 + "&from=signBuy&ReferralId=" + getParam("ReferralId")
        + "&quantityPerDay=" + quantityPerDay + "&startDate=" + startDate + "&sendDays=" + sendDays;
}